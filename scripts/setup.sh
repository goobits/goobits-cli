...
# ║                        AUTO-GENERATED FILE                               ║
# ║                                                                           ║
# ║  Generated by: goobits-cli v3.0.1                     ║
# ║  Generated from:                                          ║
# ║                                                                           ║
...
install_with_pip() {
    ...
    if [[ "$install_dev" == "true" ]]; then
        ...
        (cd "$PROJECT_DIR" && python3 -m pip install --editable "$DEVELOPMENT_PATH[dev,test]" $PIP_USER_FLAG) &
        show_spinner $!
        wait $!
        local exit_code=$?
        ...
        python3 -m pip install "$PYPI_NAME[dev,test]" $PIP_USER_FLAG &
        show_spinner $!
        wait $!
        local exit_code=$?
...
upgrade_package() {
    ...
    if [[ "$PIPX_AVAILABLE" == "true" ]]; then
        ...
        python3 -m pip install --upgrade "$PYPI_NAME[dev,test]" $PIP_USER_FLAG >/dev/null 2>&1 &
        show_spinner $!
        wait $!
        local exit_code=$?
    fi

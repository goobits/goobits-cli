 ...
 * Test Node.js CLI CLI - Node.js Implementation
 */
...
async function builtinUpgradeCommand(checkOnly = false, pre = false, version = null, dryRun = false) {
    if (checkOnly) {
        console.log(`Checking for updates to Test Node.js CLI...`);
    ...
    if (!setupScript) {
        ...
        console.log(`Enhanced setup script not found. Using basic upgrade for Test Node.js CLI...`);
        ...
        try {
            ...
            console.log(`✅ Test Node.js CLI upgraded successfully!`);
            console.log(`Run 'testnode --version' to verify the new version.`);
        } catch (e) {
...
async function loadPlugins(program) {
    const pluginDirs = [
        path.join(process.env.HOME || process.env.USERPROFILE, '.config', 'goobits', 'testnode', 'plugins'),
...
program
    .name('testnode')
    .version(getVersion())
    .description(`testnode v${getVersion()} - Test Node.js CLI`)
...
A test CLI for Node.js generation
...
builtin_completionCmd
    ...
    .action((shell) => {
        ...
        console.log(`   testnode completion generate ${shell} > completion.${shell}`);

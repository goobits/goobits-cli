#!/bin/bash
# Enhanced setup script for Goobits CLI Framework
...
install_system_dependencies() {
    local apt_packages=("git" "python3-dev" "curl" "wget" "pipx")
...
setup_config_dir() {
    ...
    case "$OSTYPE" in
        darwin*)
            config_dir="$HOME/Library/Application Support/goobits-cli"
            ;;
        msys*|win32*)
            config_dir="${APPDATA:-$HOME}/goobits-cli"
            ;;
        *)
            config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/goobits-cli"
            ;;
    esac
...
uninstall() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/goobits-cli"
        if [ -d "$config_dir" ]; then
...
show_help() {
    echo ""
    echo -e "${BLUE}Goobits CLI Framework Setup Script${NC}"
    echo "===================================="
    echo ""

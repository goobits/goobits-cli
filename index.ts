 ...
 * Main CLI implementation for NodeJSTestCLI
 * 
 * Features:
...
// Import decorator system (optional) - loaded dynamically when needed
let registerDecoratorCommands: any = null;

// Helper function to load decorator system
async function loadDecoratorSystem(): Promise<void> {
  try {
    // Use dynamic import to avoid compile-time module resolution errors
    const decoratorModule = await import('./examples/decorators.js').catch(() => null);
    if (decoratorModule) {
      registerDecoratorCommands = decoratorModule.registerDecoratorCommands;
    }
  } catch {
...
async function loadPlugins(program: Command): Promise<Result<number, PluginError[]>> {
  const pluginDirs: string[] = [
    ...
    join(process.env.HOME || '', '.config', 'goobits', 'NodeJSTestCLI', 'plugins'),
...
async function builtinUpgradeCommand(options: UpgradeOptions): Promise<void> {
  ...
  const packageName: string = 'nodejs-test-cli';
  const displayName: string = 'NodeJSTestCLI';
  ...
  try {
    ...
    console.log(chalk.gray(`Run 'nodejstestcli --version' to verify the new version.`));
  } catch (error: any) {
...
'default'    );
    ...
    cmd.action(async (name: unknown, options: Record<string, unknown>, command: Command) => {
      ...
      if (appHooks && appHooks[hookName]) {
        ...
        const globalOptions: GlobalOptions = {
          ...
          config: command.parent?.opts().config as string,
        };
...
;
    ...
    cmd.action(async (environment: unknown, options: Record<string, unknown>, command: Command) => {
      ...
      const commandInstanceName: string = 'deployCommand';
      if (appHooks && appHooks[commandInstanceName]) {
        ...
        console.error(chalk.red(`Error: Managed command 'deploy' requires '${commandInstanceName}' export in app_hooks.js`));
        process.exit(1);
      }
    });
  }
...
8080      );
      ...
      subCmd.action(async (options: any, command: Command) => {
        const hookName: string = 'onServerStart';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
          ...
          // Default placeholder behavior
          console.log(chalk.blue(`Executing start subcommand...`));
          console.log('Options:', options);
        }
      });
    }
    {
      ...
      subCmd.action(async (options: any, command: Command) => {
        const hookName: string = 'onServerStop';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
          ...
          // Default placeholder behavior
          console.log(chalk.blue(`Executing stop subcommand...`));
          console.log('Options:', options);
        }
      });
    }
    {
      ...
      subCmd.action(async (service: any, options: any, command: Command) => {
        const hookName: string = 'onServerRestart';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
          ...
          // Default placeholder behavior
          console.log(chalk.blue(`Executing restart subcommand...`));
...
'up'      );
      ...
      subCmd.action(async (options: any, command: Command) => {
        const hookName: string = 'onDatabaseMigrate';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
          ...
          // Default placeholder behavior
          console.log(chalk.blue(`Executing migrate subcommand...`));
          console.log('Options:', options);
        }
      });
    }
    {
      ...
      subCmd.action(async (dataset: any, options: any, command: Command) => {
        const hookName: string = 'onDatabaseSeed';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
          ...
          // Default placeholder behavior
          console.log(chalk.blue(`Executing seed subcommand...`));
...
'./backup.sql'      );
      ...
      subCmd.action(async (options: any, command: Command) => {
        const hookName: string = 'onDatabaseBackup';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
          ...
          // Default placeholder behavior
          console.log(chalk.blue(`Executing backup subcommand...`));
          console.log('Options:', options);
        }
      });
    }
...
;
    ...
    cmd.action(async (pattern: unknown, options: Record<string, unknown>, command: Command) => {
      // Standard command - use hook pattern
      const hookName: string = 'onTest';
      if (appHooks && appHooks[hookName]) {
        const hookFunc = appHooks[hookName];
        ...
        // Default placeholder behavior
        console.log(chalk.blue(`Executing test command...`));

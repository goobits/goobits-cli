#!/bin/bash
# Enhanced setup script for TestNodeCLI
# Generated by goobits-cli

set -e  # Exit on error

# Configuration
PACKAGE_NAME="test-node-cli"
COMMAND_NAME="testnodecli"
DISPLAY_NAME="TestNodeCLI"
VERSION="2.0.0-beta.1"
DESCRIPTION="Node.js test CLI for validation"
NPM_NAME="test-node-cli"
...
is_development() {
    [ -f "package.json" ] && [ -f "testnodecli.js" -o -f "cli.js" ]
}
...
setup_config_dir() {
    ...
    case "$OSTYPE" in
        darwin*)
            config_dir="$HOME/Library/Application Support/test-node-cli"
            ;;
        msys*|win32*)
            config_dir="${APPDATA:-$HOME}/test-node-cli"
            ;;
        *)
            config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/test-node-cli"
            ;;
    esac
...
uninstall() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/test-node-cli"
        if [ -d "$config_dir" ]; then
...
show_help() {
    echo ""
    echo -e "${BLUE}TestNodeCLI Setup Script${NC}"
    echo "===================================="
    echo ""

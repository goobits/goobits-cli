#!/bin/bash
# Enhanced TypeScript setup script for TestTSCLI
...
setup_config_dir() {
    ...
    case "$OSTYPE" in
        darwin*)
            config_dir="$HOME/Library/Application Support/test-ts-cli"
            ;;
        msys*|win32*)
            config_dir="${APPDATA:-$HOME}/test-ts-cli"
            ;;
        *)
            config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/test-ts-cli"
            ;;
    esac
...
uninstall() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/test-ts-cli"
        if [ -d "$config_dir" ]; then
...
show_help() {
    echo ""
    echo -e "${BLUE}TestTSCLI TypeScript Setup Script${NC}"
    echo "====================================="
    echo ""

#!/bin/bash
# Enhanced setup script for Test 3-Level Commands
# Generated by goobits-cli

set -e  # Exit on error

# Configuration
PACKAGE_NAME="test-3level"
COMMAND_NAME="test3level"
DISPLAY_NAME="Test 3-Level Commands"
VERSION="1.2.3"
DESCRIPTION="Test 3-level nested commands"
NPM_NAME="test-3level"
...
setup_config_dir() {
    ...
    case "$OSTYPE" in
        darwin*)
            config_dir="$HOME/Library/Application Support/test-3level"
            ;;
        msys*|win32*)
            config_dir="${APPDATA:-$HOME}/test-3level"
            ;;
        *)
            config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/test-3level"
            ;;
    esac
...
uninstall() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/test-3level"
        if [ -d "$config_dir" ]; then
...
show_help() {
    echo ""
    echo -e "${BLUE}Test 3-Level Commands Setup Script${NC}"
    echo "===================================="
    echo ""

#!/bin/bash
# Enhanced setup script for Nested Command Demo
...
setup_config_dir() {
    ...
    case "$OSTYPE" in
        darwin*)
            config_dir="$HOME/Library/Application Support/nested-demo"
            ;;
        msys*|win32*)
            config_dir="${APPDATA:-$HOME}/nested-demo"
            ;;
        *)
            config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/nested-demo"
            ;;
    esac
...
uninstall() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/nested-demo"
        if [ -d "$config_dir" ]; then
...
show_help() {
    echo ""
    echo -e "${BLUE}Nested Command Demo Setup Script${NC}"
    echo "===================================="
    echo ""

 ...
 * Nested Command Demo CLI - Node.js Implementation
 */
...
async function builtinUpgradeCommand(checkOnly = false, pre = false, version = null, dryRun = false) {
    if (checkOnly) {
        console.log(`Checking for updates to Nested Command Demo...`);
        console.log('Update check not yet implemented. Run without --check to upgrade.');
        return;
    }

    if (dryRun) {
        console.log('Dry run - would execute: npm update -g nested-demo');
        return;
    }
    ...
    const searchPaths = [
        ...
        path.join(__dirname, '../setup.sh'),
        path.join(process.env.HOME, '.local', 'share', 'nested-demo', 'setup.sh')
    ];
    ...
    if (!setupScript) {
        ...
        console.log(`Enhanced setup script not found. Using basic upgrade for Nested Command Demo...`);
        
        const packageName = 'nested-demo';
        const npmName = 'nested-demo';
        
        try {
            const cmd = process.platform === 'win32' ? 'npm.cmd' : 'npm';
            execSync(`${cmd} update -g ${npmName}`, { stdio: 'inherit' });
            console.log(`✅ Nested Command Demo upgraded successfully!`);
            console.log(`Run 'demo --version' to verify the new version.`);
        } catch (e) {
...
async function loadPlugins(program) {
    const pluginDirs = [
        path.join(process.env.HOME || process.env.USERPROFILE, '.config', 'goobits', 'demo', 'plugins'),
...
program
    .name('demo')
    .version(getVersion())
    .description(`demo v${getVersion()} - Deep nesting demo`)
    ...
    .action(async (action, options, command) => {
        try {
            ...
            if (hooks && typeof hooks[hookName] === 'function') {
                ...
                // Default placeholder behavior
                console.log(`Executing users command...`);
...
builtin_completionCmd
    ...
    .action((shell) => {
        ...
        console.log(`   demo completion generate ${shell} > completion.${shell}`);

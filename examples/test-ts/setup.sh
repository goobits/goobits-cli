#!/bin/bash
# Enhanced TypeScript setup script for Test TypeScript
...
setup_config_dir() {
    ...
    case "$OSTYPE" in
        darwin*)
            config_dir="$HOME/Library/Application Support/test_ts"
            ;;
        msys*|win32*)
            config_dir="${APPDATA:-$HOME}/test_ts"
            ;;
        *)
            config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/test_ts"
            ;;
    esac
...
uninstall() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/test_ts"
        if [ -d "$config_dir" ]; then
...
show_help() {
    echo ""
    echo -e "${BLUE}Test TypeScript TypeScript Setup Script${NC}"
    echo "====================================="
    echo ""

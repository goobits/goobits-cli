 ...
 * Main CLI implementation for Demo Node.js CLI
 */
...
const loadPlugins = asyncErrorHandler(async function loadPlugins(program) {
  try {
    ...
    const pluginDirs = [
      ...
      join(process.env.HOME || '', '.config', 'goobits', 'demo_js', 'plugins'),
...
async function builtinUpgradeCommand(options) {
  ...
  try {
    ...
    console.log(chalk.gray(`Run 'demo_js --version' to verify the new version.`));
  } catch (error) {
...
function generateDynamicCompletion(shell) {
  ...
  switch (shell) {
    case 'bash':
      console.log(`# Basic bash completion for demo_js
...
complete -F _demo_js_completions demo_js`);
    ...
    case 'zsh':
      console.log(`#compdef demo_js
# Basic zsh completion for demo_js
_demo_js() {
    local commands=(
...
_demo_js "$@"`);
    ...
    case 'fish':
      console.log(`# Basic fish completion for demo_js
complete -c demo_js -f
complete -c demo_js -l help -s h -d 'Show help information'
complete -c demo_js -l version -s V -d 'Show version information'

${commands.split(' ').map(cmd => 
  `complete -c demo_js -n '__fish_use_subcommand' -a ${cmd} -d '${cmd} command'`
).join('\n')}`);
      break;
  }
}

#!/bin/bash
# Enhanced TypeScript setup script for Demo TypeScript CLI
...
setup_config_dir() {
    ...
    case "$OSTYPE" in
        darwin*)
            config_dir="$HOME/Library/Application Support/demo-typescript-cli"
            ;;
        msys*|win32*)
            config_dir="${APPDATA:-$HOME}/demo-typescript-cli"
            ;;
        *)
            config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/demo-typescript-cli"
            ;;
    esac
...
uninstall() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/demo-typescript-cli"
        if [ -d "$config_dir" ]; then
...
show_help() {
    echo ""
    echo -e "${BLUE}Demo TypeScript CLI TypeScript Setup Script${NC}"
    echo "====================================="
    echo ""

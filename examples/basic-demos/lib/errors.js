...
export class PluginNotFoundError extends CLIError {
  constructor(pluginName, searchPaths = []) {
    const pathsInfo = searchPaths.length > 0 
      ? `\n\nSearched in: ${searchPaths.join(', ')}`
      : '';
    super(`Plugin not found: ${pluginName}${pathsInfo}`, EXIT_CODES.PLUGIN_ERROR);
    this.pluginName = pluginName;
...
export class PluginError extends CLIError {
  constructor(message, pluginName = null, cause = null) {
    super(`Plugin Error: ${message}`, EXIT_CODES.PLUGIN_ERROR, cause);
    this.pluginName = pluginName;
  }
}
...
export class FileSystemError extends CLIError {
  constructor(message, path, operation = null) {
    super(`File System Error: ${message}`, EXIT_CODES.GENERAL_ERROR);
    this.path = path;
...
export class ProcessError extends CLIError {
  constructor(command, exitCode, message) {
    super(`Process Error: ${message}`, EXIT_CODES.GENERAL_ERROR);
    this.command = command;

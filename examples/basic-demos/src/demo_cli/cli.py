...
║  Generated from: python-example.yaml            ║
║  Generated on: 2025-08-25 23:58:38            ║
║                                                                              ║
...
def _path_to_import_path(file_path: str, package_name: str = "demo_python_cli") -> tuple[str, str]:
...
# No hooks path configured, try default locations
    ...
    try:
        return importlib.import_module(f"demo_python_cli.{default_module_name}")
    except ImportError:
        pass
    
    # Strategy 2: Try relative import
    try:
        return importlib.import_module(f".{default_module_name}", package="demo_python_cli")
    except ImportError:
        pass
...
def get_version():
    ...
    try:
        ...
        try:
            ...
            return version("demo-python-cli")
        ...
        try:
            ...
            return pkg_resources.get_distribution("demo-python-cli").version
        except:
            pass
...
class VersionedRichGroup(RichGroup):
    def format_usage(self, ctx, formatter):
        ...
        formatter.write_usage(f"demo_py v{get_version()}", " ".join(pieces))
...
@click.version_option(version=get_version(), prog_name=f"demo_py v{get_version()}")
@click.option(
    "--verbose", "-v",
...
def main(ctx, verbose, interactive):
    """Python CLI demonstration

    \b    [italic #B3B8C0]None[/italic #B3B8C0]
    
    [italic #B3B8C0]A sample Python CLI built with Goobits[/italic #B3B8C0]    
    """
    pass
...
def upgrade(check, version, pre, dry_run):
    """Upgrade Demo Python CLI to the latest version"""
    import subprocess
    import shutil
    from pathlib import Path
    
    package_name = "demo-python-cli"
    command_name = "demo_py"
    display_name = "Demo Python CLI"
...
@click.option(
    ...
    default='en')
@click.pass_context
def greet(ctx, name, message, style, count, uppercase, language):
...
@click.option(
    ...
    default='all')
@click.pass_context
def info(ctx, format, verbose, sections):
    ...
    try:
        if hooks is None:
            ...
            raise ConfigError(
                error_msg,
                suggestion=f"Create app_hooks.py with function: def on_info(...): pass"
            )

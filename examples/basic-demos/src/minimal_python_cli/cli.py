...
║  Generated from: minimal-python-example.yaml            ║
║  Generated on: 2025-08-25 18:21:03            ║
║                                                                              ║
...
def _path_to_import_path(file_path: str, package_name: str = "minimal_python_cli") -> tuple[str, str]:
...
# No hooks path configured, try default locations
    ...
    try:
        return importlib.import_module(f"minimal_python_cli.{default_module_name}")
    except ImportError:
        pass
    
    # Strategy 2: Try relative import
    try:
        return importlib.import_module(f".{default_module_name}", package="minimal_python_cli")
    except ImportError:
        pass
...
def get_version():
    ...
    try:
        ...
        try:
            ...
            return version("minimal-python-cli")
        ...
        try:
            ...
            return pkg_resources.get_distribution("minimal-python-cli").version
        except:
            pass
...
class VersionedRichGroup(RichGroup):
    def format_usage(self, ctx, formatter):
        ...
        formatter.write_usage(f"minimal_py v{get_version()}", " ".join(pieces))
...
@click.version_option(version=get_version(), prog_name=f"minimal_py v{get_version()}")
@click.option(
    "--verbose", "-v",
...
def main(ctx, verbose, interactive):
    ...
    [italic #B3B8C0]A minimal Python CLI[/italic #B3B8C0]    
    """
    pass
...
def upgrade(check, version, pre, dry_run):
    """Upgrade Minimal Python CLI to the latest version"""
    import subprocess
    import shutil
    from pathlib import Path
    
    package_name = "minimal-python-cli"
    command_name = "minimal_py"
    display_name = "Minimal Python CLI"

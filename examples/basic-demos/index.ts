...
const runtimeConfig: Partial<ConfigSchema> = {
  package_name: 'demo-typescript-cli',
  command_name: 'demo_ts',
  display_name: 'Demo TypeScript CLI',
...
async function loadPlugins(program: Command): Promise<Result<number, PluginError[]>> {
  const pluginDirs: string[] = [
    ...
    join(process.env.HOME || '', '.config', 'goobits', 'demo_ts', 'plugins'),
...
async function builtinUpgradeCommand(options: UpgradeOptions): Promise<void> {
  ...
  const packageName: string = 'demo-typescript-cli';
  const displayName: string = 'Demo TypeScript CLI';
  ...
  try {
    ...
    console.log(chalk.gray(`Run 'demo_ts --version' to verify the new version.`));
  } catch (error: any) {
...
export async function cli(): Promise<void> {
  ...
  "divide"
    ...
    cmd.action(async (operation: unknown, a: unknown, b: unknown, options: Record<string, unknown>, command: Command) => {
      
      // Standard command - use hook pattern
      const hookName: string = 'onCalculate';
  ...
  
  {
    ...
    cmd.action(async (options: Record<string, unknown>, command: Command) => {
      ...
      // Standard command - use hook pattern
      const hookName: string = 'onStatus';
      if (appHooks && appHooks[hookName]) {
        const hookFunc = appHooks[hookName];

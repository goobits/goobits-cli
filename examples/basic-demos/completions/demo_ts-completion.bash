...
_demo_ts_completions() {
    ...
    case "${prev}" in
        demo_ts)
            # Top level commands
            local commands="calculate status upgrade plugin help"
            COMPREPLY=($(compgen -W "$commands" -- "$cur"))
            return 0
            ;;
        calculate)
            # Options for calculate
            local options="--precision -p --help -h"
        ...
        *)
            ...
            while [[ $i -lt $cword ]]; do
                case "${words[$i]}" in
                    calculate)
                        in_subcommand="calculate"
                        break
                        ;;
                    status)
                        in_subcommand="status"
                        break
                        ;;
                    plugin|upgrade)
            ...
            if [[ -n "$in_subcommand" ]]; then
                ...
                case "$in_subcommand" in
                    calculate)
                        local cmd_options="--precision -p --help -h"

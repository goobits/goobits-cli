...
_demo_js_completions() {
    ...
    case "${prev}" in
        demo_js)
            # Top level commands
            local commands="greet serve upgrade plugin help"
            COMPREPLY=($(compgen -W "$commands" -- "$cur"))
            return 0
            ;;
        greet)
            # Options for greet
            local options="--style -s --help -h"
            COMPREPLY=($(compgen -W "$options" -- "$cur"))
            return 0
            ;;
        serve)
            # Options for serve
            local options="--port -p --help -h"
            COMPREPLY=($(compgen -W "$options" -- "$cur"))
            return 0
            ;;
        plugin)
        ...
        *)
            ...
            if [[ -n "$in_subcommand" ]]; then
                ...
                case "$in_subcommand" in
                    greet)
                        local cmd_options="--style -s --help -h"
                        COMPREPLY=($(compgen -W "$cmd_options" -- "$cur"))
                        ;;
                    serve)
                        local cmd_options="--port -p --help -h"
                        COMPREPLY=($(compgen -W "$cmd_options" -- "$cur"))
                        ;;
                    plugin)

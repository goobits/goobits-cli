/**
 * Built-in plugin management command for Demo Node.js CLI
 */
...
export default function registerPluginCommand(program) {
  const builtin_pluginCmd = program
    .command('plugin')
    .description('🔌 Manage plugins for Demo Node.js CLI')
    .alias('plugins');
  ...
  builtin_pluginCmd
    ...
    .action(async (pluginName, options) => {
      try {
        ...
        console.log(chalk.green(`✅ Plugin template "${pluginName}" created successfully!`));
      } catch (error) {
...
async function createFunctionPlugin(pluginDir, pluginName) {
  const pluginCode = `/**
 * ${pluginName} plugin for Demo Node.js CLI
...
export default function ${toCamelCase(pluginName)}Plugin(program) {
  ...
  program
    ...
    .action((options) => {
      console.log(chalk.blue('🔌 Plugin command executed!'));
...
A plugin for Demo Node.js CLI.

## Installation

\`\`\`bash
# Copy this plugin to your plugins directory
cp -r ${pluginName} ~/.config/goobits/demo_js/plugins/
\`\`\`

## Usage

\`\`\`bash
demo_js ${pluginName} --help
\`\`\`
...
export class ${toPascalCase(pluginName)}Plugin {
  ...
  async execute(options) {
    console.log(chalk.blue('🔌 Class plugin command executed!'));
...
A class-based plugin for Demo Node.js CLI.
...
async function createNpmPlugin(pluginDir, pluginName) {
  // Create package.json
  const packageJson = {
    name: `demo_js-plugin-${pluginName}`,
    version: '1.0.0',
    description: `${pluginName} plugin for Demo Node.js CLI`,
    main: 'index.js',
    type: 'module',
    keywords: ['demo_js-plugin', 'goobits-plugin', 'cli-plugin'],
    author: '',
    license: 'MIT',
    peerDependencies: {
      'commander': '^11.0.0',
...
export default function ${toCamelCase(pluginName)}Plugin(program) {
  program
    ...
    .option('--example <value>', 'Example option')
    .action(async (options) => {
      console.log(chalk.blue('🔌 NPM plugin command executed!'));
      console.log('Plugin name:', '${pluginName}');
      console.log('Options:', options);
...
function toCamelCase(str) {
  ...
  const readme = `# demo_js-plugin-${pluginName}
...
\`\`\`bash
npm install -g demo_js-plugin-${pluginName}
\`\`\`
...
cp -r . ~/.config/goobits/demo_js/plugins/${pluginName}/
\`\`\`
...
2. Run \`npm publish\`
3. Install with \`npm install -g demo_js-plugin-${pluginName}\`

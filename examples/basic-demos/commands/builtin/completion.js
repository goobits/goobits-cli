/**
 * Built-in shell completion command for Demo Node.js CLI
 */
  ...
  builtin_completionCmd
    ...
    .action(async (shell) => {
      try {
        ...
        console.log(chalk.green(`✅ Completion uninstalled for ${shell}`));
      } catch (error) {
...
function generateDynamicCompletion(shell) {
  ...
  switch (shell) {
    case 'bash':
      return `# Bash completion for demo_js
...
_demo_js "$@"`;
    ...
    case 'fish':
      return `# Fish completion for demo_js
complete -c demo_js -l help -s h -d 'Show help'
complete -c demo_js -l version -s V -d 'Show version'`;
...
function getCompletionPath(shell) {
  ...
  switch (shell) {
    case 'bash':
      ...
      return join(home, '.local/share/bash-completion/completions', 'demo_js');
      
    case 'zsh':
      // Use ~/.local/share/zsh/site-functions for user installations
      return join(home, '.local/share/zsh/site-functions', '_demo_js');
      
    case 'fish':
      // Use ~/.config/fish/completions
      return join(home, '.config/fish/completions', 'demo_js.fish');
...
function showInstallInstructions(shell) {
  ...
  switch (shell) {
    case 'bash':
      ...
      console.log(chalk.gray('• If using macOS, you may need to install bash-completion:'));
      console.log(chalk.gray('  brew install bash-completion'));
      break;
      
    case 'zsh':
      console.log(chalk.gray('• Add to your ~/.zshrc:'));
    ...
    case 'fish':
      console.log(chalk.gray('• Completions should be available immediately'));
      console.log(chalk.gray('• If not, restart fish or run: fish_reload'));
      break;
  }
}
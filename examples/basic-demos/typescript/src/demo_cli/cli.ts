#!/usr/bin/env node
/**
 * Multi-Language Demo CLI - A sample multi-language CLI built with Goobits
...
class ConfigManager {
    ...
    private _getDefaultConfigPath(): string {
        const configDir = join(homedir(), '.config', 'demo_multi');
    ...
    public loadConfig(): void {
        ...
        for (const key of Object.keys(this.config)) {
            const envKey = `DEMO_MULTI_${key.toUpperCase()}`;
...
const structuredFormatter = winston.format.printf(({ timestamp, level, message, ...meta }) => {
    ...
    if (isProduction) {
        ...
        return `${timestamp} ${level.toUpperCase().padEnd(8)} Multi-Language Demo CLI.${message}${contextStr}${extraStr}`;
    }
});
...
complete -F _demo_multi_completions demo_multi`;
    }
...
program
    .command('greet')
    .description('Greet someone with style')    .argument('<name>', 'Name to greet')    .argument('<message>', 'Custom greeting message')    .option(
        '-s, --style <style>',
        'Greeting style',
        "casual"    )    .option(
        '-c, --count <count>',
...
program
    .command('info')
    .description('Display system and environment information')    .option(
        '-f, --format <format>',
        'Output format',
        "text"    )    .option(
        '-v, --verbose',
        ...
        false    )    .option(
        '-s, --sections <sections>',
        'Comma-separated sections to show',
        "all"    )    .action(async (...args: any[]) => {
        try {
            // Load hooks if not already loaded
            if (!hooks) {
                await loadHooks();

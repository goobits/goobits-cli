/**
 * Progress and UI helpers for Nested Command Demo
    ...
    createSpinner(text: string = 'Loading...', spinnerType: string = 'dots'): any {
        if (this._fallbackEnabled || !HAS_ORA) {
            return {
                ...
                stop: () => console.log(' ✓'),
                succeed: (successText?: string) => console.log(` ✓${successText ? ' ' + successText : ''}`),
                fail: (failText?: string) => console.log(` ✗${failText ? ' ' + failText : ''}`),
                warn: (warnText?: string) => console.log(` ⚠${warnText ? ' ' + warnText : ''}`),
                info: (infoText?: string) => console.log(` ℹ${infoText ? ' ' + infoText : ''}`),
                text: text,
            };
        }

#!/bin/bash
# Bash completion script for Nested Command Demo (demo)
...
_demo_completions() {
    ...
    case "${prev}" in
        demo)
            # Top level commands
            local commands="simple database api upgrade plugin help"
            COMPREPLY=($(compgen -W "$commands" -- "$cur"))
            return 0
            ;;
        simple)
            # Options for simple
            local options="--verbose --help -h"
            COMPREPLY=($(compgen -W "$options" -- "$cur"))
            return 0
            ;;
        database)
            # Subcommands for database
            local subcommands="users backup "
        ...
        *)
            ...
            if [[ -n "$in_subcommand" ]]; then
                ...
                case "$in_subcommand" in
                    simple)
                        local cmd_options="--verbose --help -h"

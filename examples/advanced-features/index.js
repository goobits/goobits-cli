...
const loadPlugins = asyncErrorHandler(async function loadPlugins(program) {
  try {
    ...
    const pluginDirs = [
      ...
      join(process.env.HOME || '', '.config', 'goobits', 'demo', 'plugins'),
...
async function builtinUpgradeCommand(options) {
  ...
  const packageName = 'nested-demo';
  const displayName = 'Nested Command Demo';
  ...
  try {
    ...
    console.log(chalk.gray(`Run 'demo --version' to verify the new version.`));
  } catch (error) {
...
function generateDynamicCompletion(shell) {
  ...
  switch (shell) {
    case 'bash':
      console.log(`# Basic bash completion for demo
_demo_completions() {
...
complete -F _demo_completions demo`);
    ...
    case 'zsh':
      console.log(`#compdef demo
# Basic zsh completion for demo
_demo() {
    local commands=(
...
_demo "$@"`);
    ...
    case 'fish':
      console.log(`# Basic fish completion for demo
complete -c demo -f
...
${commands.split(' ').map(cmd => 
  `complete -c demo -n '__fish_use_subcommand' -a ${cmd} -d '${cmd} command'`
).join('\n')}`);
      break;
  }
}
...
export const cli = asyncErrorHandler(async function cli() {
  ...
  program
    .command('upgrade')
    .description('Upgrade Nested Command Demo to the latest version')
...
'table'      );
      ...
      subCmd.action(async (action, options, command) => {
        const hookName = 'onDatabaseUsers';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              ...
              `Failed to execute subcommand 'users'. ${error.message}`,
              1
            );
          ...
          // Default placeholder behavior
          console.log(chalk.blue(`Executing users subcommand...`));

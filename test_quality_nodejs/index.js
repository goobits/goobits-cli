 ...
 * Main CLI implementation for Complex CLI Test Tool
 */
...
const loadPlugins = asyncErrorHandler(async function loadPlugins(program) {
  try {
    ...
    const pluginDirs = [
      ...
      join(process.env.HOME || '', '.config', 'goobits', 'complex-cli', 'plugins'),
...
async function builtinUpgradeCommand(options) {
  ...
  const displayName = 'Complex CLI Test Tool';
  ...
  try {
    ...
    console.log(chalk.gray(`Run 'complex-cli --version' to verify the new version.`));
  } catch (error) {
...
function generateDynamicCompletion(shell) {
  ...
  switch (shell) {
    case 'bash':
      console.log(`# Basic bash completion for complex-cli
_complex_cli_completions() {
...
complete -F _complex_cli_completions complex-cli`);
      break;
      
    case 'zsh':
      console.log(`#compdef complex-cli
# Basic zsh completion for complex-cli
_complex_cli() {
    local commands=(
...
_complex_cli "$@"`);
    ...
    case 'fish':
      console.log(`# Basic fish completion for complex-cli
complete -c complex-cli -f
complete -c complex-cli -l help -s h -d 'Show help information'
complete -c complex-cli -l version -s V -d 'Show version information'

${commands.split(' ').map(cmd => 
  `complete -c complex-cli -n '__fish_use_subcommand' -a ${cmd} -d '${cmd} command'`
).join('\n')}`);
      break;
  }
}
...
export const cli = asyncErrorHandler(async function cli() {
  ...
  
  {
    ...
    cmd.action(async (target, source, options, command) => {
  ...
  "prod"
    ...
    cmd.option(
      '-f, --force',
      'Force deployment ignoring checks'
    );
    
    

    cmd.action(async (environment, options, command) => {
      ...
      if (appHooks && appHooks[hookName]) {
        ...
        try {
          ...
          const hookError = error instanceof CLIError ? error : new CLIError(
            `Hook function '${hookName}' failed: ${error.message}`,
            'HOOK_EXECUTION_ERROR',
            `Failed to execute command 'deploy'. ${error.message}`,
            1
          );
          handleError(hookError);
        }
        ...
        console.log(chalk.blue(`Executing deploy command...`));
  ...
  
  {
    ...
    
    {
      ...
      subCmd.action(async (options, command) => {
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              ...
              `Failed to execute subcommand 'unit'. ${error.message}`,
              1
            );
          ...
          console.log(chalk.blue(`Executing unit subcommand...`));
    ...
    
    {
      ...
      subCmd.action(async (service, options, command) => {
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              `Subcommand hook '${hookName}' failed: ${error.message}`,
              'SUBCOMMAND_EXECUTION_ERROR',
              `Failed to execute subcommand 'integration'. ${error.message}`,
              1
            );
            handleError(subcommandError);
          }
          ...
          console.log(chalk.blue(`Executing integration subcommand...`));
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              `Subcommand hook '${hookName}' failed: ${error.message}`,
              'SUBCOMMAND_EXECUTION_ERROR',
              `Failed to execute subcommand 'performance'. ${error.message}`,
              1
            );
            handleError(subcommandError);
          }
          ...
          console.log(chalk.blue(`Executing performance subcommand...`));
  ...
  
  {
    ...
    
    {
      ...
      subCmd.action(async (key, options, command) => {
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              `Subcommand hook '${hookName}' failed: ${error.message}`,
              'SUBCOMMAND_EXECUTION_ERROR',
              `Failed to execute subcommand 'get'. ${error.message}`,
              1
            );
            handleError(subcommandError);
          }
          ...
          console.log(chalk.blue(`Executing get subcommand...`));
    ...
    
    {
      ...
      subCmd.action(async (key, value, options, command) => {
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              `Subcommand hook '${hookName}' failed: ${error.message}`,
              'SUBCOMMAND_EXECUTION_ERROR',
              `Failed to execute subcommand 'set'. ${error.message}`,
              1
            );
            handleError(subcommandError);
          }
          ...
          console.log(chalk.blue(`Executing set subcommand...`));
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              `Subcommand hook '${hookName}' failed: ${error.message}`,
              'SUBCOMMAND_EXECUTION_ERROR',
              `Failed to execute subcommand 'list'. ${error.message}`,
              1
            );
            handleError(subcommandError);
          }
          ...
          console.log(chalk.blue(`Executing list subcommand...`));
  ...
  if (testCmd) {
    program.addHelpText('after', `  ${testCmd.name().padEnd(15)} ${testCmd.description()}`);
  }
  
  
  
  program.addHelpText('after', `\n${chalk.yellow('Operations:')}`);
  
  
  const deployCmd = program.commands.find(c => c.name() === 'deploy');
  if (deployCmd) {
    program.addHelpText('after', `  ${deployCmd.name().padEnd(15)} ${deployCmd.description()}`);
  }
  ...
  if (configCmd) {
    program.addHelpText('after', `  ${configCmd.name().padEnd(15)} ${configCmd.description()}`);
  }

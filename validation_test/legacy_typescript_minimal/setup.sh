#!/bin/bash
# Enhanced TypeScript setup script for TypeScript Test CLI
...
setup_config_dir() {
    ...
    case "$OSTYPE" in
        darwin*)
            config_dir="$HOME/Library/Application Support/typescript-test-cli"
            ;;
        msys*|win32*)
            config_dir="${APPDATA:-$HOME}/typescript-test-cli"
            ;;
        *)
            config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/typescript-test-cli"
            ;;
    esac
...
uninstall() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/typescript-test-cli"
        if [ -d "$config_dir" ]; then
...
show_help() {
    echo ""
    echo -e "${BLUE}TypeScript Test CLI TypeScript Setup Script${NC}"
    echo "====================================="
    echo ""

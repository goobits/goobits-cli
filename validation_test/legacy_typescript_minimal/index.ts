 ...
 * Main CLI implementation for TypeScript Test CLI
 * 
 * Features:
...
const runtimeConfig: Partial<ConfigSchema> = {
  package_name: 'typescript-test-cli',
  command_name: 'typescript-test',
  display_name: 'TypeScript Test CLI',
  description: 'A TypeScript test CLI for validation',
  version: '1.0.0',
};
...
async function loadPlugins(program: Command): Promise<Result<number, PluginError[]>> {
  const pluginDirs: string[] = [
    ...
    join(process.env.HOME || '', '.config', 'goobits', 'TypeScript Test CLI', 'plugins'),
...
async function builtinUpgradeCommand(options: UpgradeOptions): Promise<void> {
  ...
  const packageName: string = 'typescript-test-cli';
  const displayName: string = 'TypeScript Test CLI';
  ...
  try {
    ...
    console.log(chalk.gray(`Run 'typescript-test --version' to verify the new version.`));
  } catch (error: any) {

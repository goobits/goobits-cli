/**
 * Auto-generated from test_config_nodejs.yaml
 * Main CLI implementation for NodeJS Test CLI
 */
...
const loadPlugins = asyncErrorHandler(async function loadPlugins(program) {
  try {
    ...
    const pluginDirs = [
      ...
      join(process.env.HOME || '', '.config', 'goobits', 'NodeJS Test CLI', 'plugins'),
...
async function builtinUpgradeCommand(options) {
  ...
  const displayName = 'NodeJS Test CLI';
  ...
  try {
    ...
    console.log(chalk.gray(`Run 'nodejs-test --version' to verify the new version.`));
  } catch (error) {
...
function generateDynamicCompletion(shell) {
  ...
  switch (shell) {
    case 'bash':
      console.log(`# Basic bash completion for nodejs-test
_nodejs_test_completions() {
...
complete -F _nodejs_test_completions nodejs-test`);
      break;
      
    case 'zsh':
      console.log(`#compdef nodejs-test
# Basic zsh completion for nodejs-test
_nodejs_test() {
    local commands=(
...
_nodejs_test "$@"`);
    ...
    case 'fish':
      console.log(`# Basic fish completion for nodejs-test
complete -c nodejs-test -f
complete -c nodejs-test -l help -s h -d 'Show help information'
complete -c nodejs-test -l version -s V -d 'Show version information'

${commands.split(' ').map(cmd => 
  `complete -c nodejs-test -n '__fish_use_subcommand' -a ${cmd} -d '${cmd} command'`
).join('\n')}`);
      break;
  }
}
...
export const cli = asyncErrorHandler(async function cli() {
  ...
  program
    .name('NodeJS Test CLI')
    .description(`A simple NodeJS test CLI\n\nBasic NodeJS CLI for validation testing`)
    .version(version)
  ...
  program
    .command('upgrade')
    .description('Upgrade NodeJS Test CLI to the latest version')
      ...
      if (appHooks && appHooks[hookName]) {
        ...
        try {
          ...
          const hookError = error instanceof CLIError ? error : new CLIError(
            ...
            `Failed to execute command 'hello'. ${error.message}`,
            1
          );

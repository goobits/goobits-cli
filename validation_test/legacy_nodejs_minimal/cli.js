 ...
 * NodeJS Test CLI CLI - Node.js Implementation
 */
...
async function builtinUpgradeCommand(checkOnly = false, pre = false, version = null, dryRun = false) {
    if (checkOnly) {
        console.log(`Checking for updates to NodeJS Test CLI...`);
    ...
    if (!setupScript) {
        ...
        console.log(`Enhanced setup script not found. Using basic upgrade for NodeJS Test CLI...`);
        ...
        try {
            ...
            console.log(`✅ NodeJS Test CLI upgraded successfully!`);
            console.log(`Run 'nodejs-test --version' to verify the new version.`);
        } catch (e) {
...
async function loadPlugins(program) {
    const pluginDirs = [
        path.join(process.env.HOME || process.env.USERPROFILE, '.config', 'goobits', 'NodeJS Test CLI', 'plugins'),
...
program
    ...
    .version(getVersion())
    .description(`NodeJS Test CLI v${getVersion()} - A simple NodeJS test CLI`)
...
completionCmd
    ...
    .action((shell) => {
        ...
        console.log(`   NodeJS Test CLI completion generate ${shell} > completion.${shell}`);

/**
 * Enhanced Plugin Manager for NodeJS Test CLI
...
export class PluginManager {
  ...
  _getPluginDirectories() {
    ...
    return [
      ...
      join(userConfigDir, 'goobits', 'NodeJS Test CLI', 'plugins'),
  ...
  async _discoverNpmPlugins(discovered) {
    try {
      const pluginPattern = `NodeJS Test CLI-plugin-*`;
  ...
  _isValidPlugin(pkg) {
    return (
      pkg.keywords && 
      (pkg.keywords.includes('NodeJS Test CLI-plugin') || 

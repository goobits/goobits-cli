#!/usr/bin/env python3
"""Auto-generated from test_config_minimal.yaml"""
import os
import sys
import logging
...
def builtin_upgrade_command(check_only=False, pre=False, version=None, dry_run=False):
    """Built-in upgrade function for Minimal Test CLI - uses enhanced setup.sh script."""
    import subprocess
    import sys
    from pathlib import Path

    if check_only:
        print(f"Checking for updates to Minimal Test CLI...")
        print("Update check not yet implemented. Run without --check to upgrade.")
        return

    if dry_run:
        print("Dry run - would execute: pipx upgrade minimal-test-cli")
        return
    ...
    search_paths = [
        ...
        Path.home() / ".local" / "share" / "minimal-test-cli" / "setup.sh",  # User data
    ...
    if setup_script is None:
        ...
        print(f"Enhanced setup script not found. Using basic upgrade for Minimal Test CLI...")
        import shutil
        
        package_name = "minimal-test-cli"
        pypi_name = "minimal-test-cli"
        ...
        if result.returncode == 0:
            print(f"✅ Minimal Test CLI upgraded successfully!")
            print(f"Run 'minimal-test --version' to verify the new version.")
        else:
...
def load_plugins(cli_group):
    ...
    plugin_dirs = [
        ...
        Path.home() / ".config" / "minimal-test-cli" / "plugins",
...
@click.version_option(version=get_version(), prog_name="Minimal Test CLI")
@click.pass_context
...
def main(ctx):
    """[bold color(6)]Minimal Test CLI v1.0.0[/bold color(6)] - A simple test CLI

    
    \b
    [#B3B8C0]Basic CLI for validation testing[/#B3B8C0]
...
def upgrade(check, version, pre, dry_run):
    """Upgrade Minimal Test CLI to the latest version."""
...
def instructions(shell):
    ...
    click.echo(f"   minimal-test-cli completion generate {shell} > completion.{shell}")

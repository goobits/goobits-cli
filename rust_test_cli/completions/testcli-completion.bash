#!/bin/bash
# Bash completion script for Test Rust CLI (testcli)
...
_testcli_completions() {
    ...
    case "${prev}" in
        testcli)
            # Top level commands
            local commands="hello info upgrade plugin help"
        ...
        hello)
            ...
            # Options for hello
            local options="--uppercase -u --help -h"
        ...
        info)
            ...
            # Options for info
            local options="--detailed -d --help -h"
            COMPREPLY=($(compgen -W "$options" -- "$cur"))
            ...
            if [[ -n "$in_subcommand" ]]; then
                ...
                case "$in_subcommand" in
                    ...
                    hello)
                        ...
                        local cmd_options="--uppercase -u --help -h"
                    ...
                    info)
                        ...
                        local cmd_options="--detailed -d --help -h"
                        COMPREPLY=($(compgen -W "$cmd_options" -- "$cur"))
...
complete -F _testcli_completions Test CLI


# Support for cargo installation paths
if [[ -n "${CARGO_HOME:-}" ]]; then
    complete -F _testcli_completions "${CARGO_HOME}/bin/testcli"
fi

# Support for default cargo binary location
if [[ -d "$HOME/.cargo/bin" ]]; then
    complete -F _testcli_completions "$HOME/.cargo/bin/testcli"
fi
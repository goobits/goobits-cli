 ...
 * CLI executable entry point for Test TypeScript CLI
...
async function runCLI(): Promise<void> {
  try {
    // Try to find the main CLI file - could be index.js or cli.js
    ...
    if (!mainPath) {
      const error = createSystemError(
        `Main CLI file not found. Searched: ${possiblePaths.join(', ')}`,
        {
          metadata: {
            operation: 'load',
    ...
    // Execute the CLI
    console.debug && console.debug(chalk.gray(`Starting Test TypeScript CLI...`));
    await cliModule.cli();
    console.debug && console.debug(chalk.gray(`Test TypeScript CLI completed successfully`));
    ...
    console.error(chalk.red.bold('✗ Failed to start Test TypeScript CLI'));
    ...
    console.error(chalk.cyan('  • Ensure Test TypeScript CLI is properly installed'));
    ...
    console.error(chalk.cyan('  • Try reinstalling: npm install -g test-typescript-cli'));

/**
 * Hook functions for Simple Greeting CLI - TypeScript version
 */
...
interface GoodbyeArgs {
    name: string;
    polite?: boolean;
    verbose?: boolean;
}

interface IntroduceArgs {
    name: string;
    role?: string;
    verbose?: boolean;
}
...
export async function onHello(args: HelloArgs): Promise<void> {
    ...
    // Define greeting styles with colors and proper typing
    const greetings: Record<NonNullable<HelloArgs['style']>, string> = {
        casual: chalk.blue(`Hey ${name}! 👋`),
        formal: chalk.green(`Good day, ${name}.`),
        enthusiastic: chalk.yellow.bold(`🎉 HELLO ${name.toUpperCase()}! 🎉`)
    };
    
    const greeting = greetings[style];
...
export async function onGoodbye(args: GoodbyeArgs): Promise<void> {
...
export async function onIntroduce(args: IntroduceArgs): Promise<void> {

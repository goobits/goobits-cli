...
Generated from: test-python.yaml
...
def _path_to_import_path(file_path: str, package_name: str = "test_python_cli") -> tuple[str, str]:
...
# No hooks path configured, try default locations
    ...
    try:
        return importlib.import_module(f"test_python_cli.{default_module_name}")
    except ImportError:
        pass
    
    # Strategy 2: Try relative import
    try:
        return importlib.import_module(f".{default_module_name}", package="test_python_cli")
    except ImportError:
        pass
...
def get_version():
    ...
    try:
        ...
        try:
            ...
            return version("test-python-cli")
        ...
        try:
            ...
            return pkg_resources.get_distribution("test-python-cli").version
        except:
            pass
...
class VersionedRichGroup(RichGroup):
    def format_usage(self, ctx, formatter):
        ...
        formatter.write_usage(f"testpy v{get_version()}", " ".join(pieces))
...
@click.version_option(version=get_version(), prog_name=f"testpy v{get_version()}")
@click.option(
    "--verbose", "-v",
...
def main(ctx, verbose, interactive):
    ...
    \b
    [italic #B3B8C0]A test CLI for Python generation[/italic #B3B8C0]
...
def upgrade(check, version, pre, dry_run):
    """Upgrade Test Python CLI to the latest version"""
    import subprocess
    import shutil
    from pathlib import Path
...
# Level 2 command: build project
@click.command()
@click.pass_context
def build_project_cmd(ctx):
    """Build a project"""
    # Enhanced error handling for Python CLI
    verbose_path = ['verbose']
    current_ctx = ctx
    ...
    try:
        if hooks is None:
            ...
            raise ConfigError(
                error_msg,
                suggestion=f"Create app_hooks.py with function: def on_build_project(...): pass"
            )
        ...
        # Handle return codes
        if isinstance(result, int) and result != 0:
            click.echo(f"Command 'build project' failed with exit code {result}", err=True)
            sys.exit(result)
...
# Attach project to build
build.add_command(build_project_cmd, 'project')

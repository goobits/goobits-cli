#!/usr/bin/env node
/**
 * Enhanced TypeScript Interactive Mode for NodeJSTestCLI
...
import { CommandContext, HookRegistry } from './types/cli.js';
...
class NodejstestcliInteractive {
...
argTypes: ['string'],
optionTypes: {
'template': 'str','skip-install': 'flag'            },
examples: [
'init <name>'
]
        });
...
'server'
        ...
        this.registerCommand({
            ...
            handler: this.handleDatabase.bind(this),
examples: [
'database'
]
        });
...
argTypes: ['string'],
optionTypes: {
'coverage': 'flag','watch': 'flag','bail': 'flag'            },
examples: [
'test <pattern>'
]
        });
    ...
    private setupEnhancedFeatures(): void {
        ...
        console.log('🎯 Enhanced TypeScript features enabled:');
        console.log('   • Type-aware tab completion');
        console.log('   • Expression evaluation with type checking');
        console.log('   • Advanced error diagnostics');
        console.log();
    }

    private loadCommandHistory(): void {
        const historyFile = path.join(process.cwd(), '.NodeJSTestCLI_history');
        try {
    ...
    start(): void {
        console.log('🚀 Welcome to NodeJSTestCLI interactive mode');
    ...
    private handleTypes(args: string[]): void {
        if (!hasTypeScriptCompiler || !this.typeChecker) {
            console.log('❌ TypeScript compiler not available for type information');
            return;
        }
    ...
    private async handleEval(args: string[]): Promise<void> {
        ...
        if (hasTypeScriptCompiler && this.typeChecker) {
            ...
            try {
                ...
                console.log(`🏷️  Type: ${typeof result}`);
            } catch (error) {
    ...
    private async handleInit(args: string[]): Promise<void> {
        const hookName = 'onInit' as keyof HookRegistry;
        const hook = hooks[hookName];
        
        if (typeof hook === 'function') {
            try {
                // Parse arguments with type validation
                const parsedContext = await this.parseCommandContext('init', args);
                await (hook as Function)(parsedContext);
            } catch (error) {
                console.error(`❌ Error in ${hookName}:`, (error as Error).message);
                
                // Enhanced error handling
                if (hasTypeScriptCompiler && this.typeChecker) {
                    const suggestions = this.typeChecker.getSuggestions(error as Error, 'init', args);
                    if (suggestions.length > 0) {
                        console.log('💡 Suggestions:');
                        suggestions.forEach(suggestion => console.log(`   • ${suggestion}`));
                    }
    ...
    private async handleDeploy(args: string[]): Promise<void> {
        const hookName = 'onDeploy' as keyof HookRegistry;
        const hook = hooks[hookName];
        
        if (typeof hook === 'function') {
            try {
                // Parse arguments with type validation
                const parsedContext = await this.parseCommandContext('deploy', args);
                await (hook as Function)(parsedContext);
            } catch (error) {
                console.error(`❌ Error in ${hookName}:`, (error as Error).message);
                ...
                // Enhanced error handling
                if (hasTypeScriptCompiler && this.typeChecker) {
                    const suggestions = this.typeChecker.getSuggestions(error as Error, 'deploy', args);
                    if (suggestions.length > 0) {
                        console.log('💡 Suggestions:');
                        suggestions.forEach(suggestion => console.log(`   • ${suggestion}`));
                    }
    ...
    private async handleServer(args: string[]): Promise<void> {
        const hookName = 'onServer' as keyof HookRegistry;
        const hook = hooks[hookName];
        
        if (typeof hook === 'function') {
            try {
                // Parse arguments with type validation
                const parsedContext = await this.parseCommandContext('server', args);
                await (hook as Function)(parsedContext);
            } catch (error) {
                console.error(`❌ Error in ${hookName}:`, (error as Error).message);
                ...
                // Enhanced error handling
                if (hasTypeScriptCompiler && this.typeChecker) {
                    const suggestions = this.typeChecker.getSuggestions(error as Error, 'server', args);
                    if (suggestions.length > 0) {
                        console.log('💡 Suggestions:');
                        suggestions.forEach(suggestion => console.log(`   • ${suggestion}`));
                    }
    ...
    private async handleDatabase(args: string[]): Promise<void> {
        const hookName = 'onDatabase' as keyof HookRegistry;
        const hook = hooks[hookName];
        
        if (typeof hook === 'function') {
            try {
                // Parse arguments with type validation
                const parsedContext = await this.parseCommandContext('database', args);
                await (hook as Function)(parsedContext);
            } catch (error) {
                console.error(`❌ Error in ${hookName}:`, (error as Error).message);
                ...
                // Enhanced error handling
                if (hasTypeScriptCompiler && this.typeChecker) {
                    const suggestions = this.typeChecker.getSuggestions(error as Error, 'database', args);
                    if (suggestions.length > 0) {
                        console.log('💡 Suggestions:');
                        suggestions.forEach(suggestion => console.log(`   • ${suggestion}`));
                    }
    ...
    private async handleTest(args: string[]): Promise<void> {
        const hookName = 'onTest' as keyof HookRegistry;
        const hook = hooks[hookName];
        
        if (typeof hook === 'function') {
            try {
                // Parse arguments with type validation
                const parsedContext = await this.parseCommandContext('test', args);
                await (hook as Function)(parsedContext);
            } catch (error) {
                console.error(`❌ Error in ${hookName}:`, (error as Error).message);
                ...
                // Enhanced error handling
                if (hasTypeScriptCompiler && this.typeChecker) {
                    const suggestions = this.typeChecker.getSuggestions(error as Error, 'test', args);
                    if (suggestions.length > 0) {
                        console.log('💡 Suggestions:');
                        suggestions.forEach(suggestion => console.log(`   • ${suggestion}`));
                    }
...
export function runInteractive(): void {
    const interactive = new NodejstestcliInteractive();
    interactive.start();
}

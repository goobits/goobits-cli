#!/usr/bin/env node
/**
 * Single Language Test CLI - Testing single language generation (backward compatibility)
...
class ConfigManager {
    ...
    private _getDefaultConfigPath(): string {
        const configDir = join(homedir(), '.config', 'testsingle');
    ...
    public loadConfig(): void {
        ...
        for (const key of Object.keys(this.config)) {
            const envKey = `TESTSINGLE_${key.toUpperCase()}`;
...
const structuredFormatter = winston.format.printf(({ timestamp, level, message, ...meta }) => {
    ...
    if (isProduction) {
        ...
        return `${timestamp} ${level.toUpperCase().padEnd(8)} Single Language Test CLI.${message}${contextStr}${extraStr}`;
    }
});
...
complete -F _testsingle_completions testsingle`;
    }
...
program
    .command('hello')
    .description('Say hello')    .argument('<name>', 'Name to greet')    .action(async (...args: any[]) => {
        try {

...
interface HookModule {    onHello?: (...args: any[]) => Promise<void>;    onBuild?: (...args: any[]) => Promise<void>;    onProject?: (...args: any[]) => Promise<void>;    onServe?: (...args: any[]) => Promise<void>;}
...
class ConfigManager {
    ...
    private _getDefaultConfigPath(): string {
        const configDir = join(homedir(), '.config', 'testts');
    ...
    public loadConfig(): void {
        ...
        for (const key of Object.keys(this.config)) {
            const envKey = `TESTTS_${key.toUpperCase()}`;
...
const structuredFormatter = winston.format.printf(({ timestamp, level, message, ...meta }) => {
    ...
    if (isProduction) {
        ...
        return `${timestamp} ${level.toUpperCase().padEnd(8)} Test TypeScript CLI.${message}${contextStr}${extraStr}`;
    }
});
...
program
    .command('hello')
    .description('Say hello')    .action(async (...args: any[]) => {
        try {
...
const buildCommand = program
    .command('build')
    .description('Build something')    .action(async (...args: any[]) => {
        try {
            // Load hooks if not already loaded
            if (!hooks) {
                await loadHooks();
            ...
            errorHandler.handleError(error, 'build command');
        }
    });
...
buildCommand
    ...
    .action(async (...args: any[]) => {
        try {
            ...
            errorHandler.handleError(error, 'project command');
        }
    });
...
program
    .command('serve')
    .description('Start server')    .action(async (...args: any[]) => {
        try {
            // Load hooks if not already loaded
            if (!hooks) {
                await loadHooks();
            ...
            errorHandler.handleError(error, 'serve command');
        }
    });

...
program
    ...
    .version('1.0.0')    .option('-v, --verbose', 'Enable verbose error output and debugging information', false);program.option(
    '--interactive, -i',
    'None');
...
helloCmd.action(async (options: any) => {
...
build_projectCmd.action(async (options: any) => {
    const globalOpts = program.opts();
    const verbose: boolean = globalOpts.verbose || false;
    
    try {
        const hookName = 'on_build_project' as keyof typeof hooks;
...
serveCmd.action(async (options: any) => {
    const globalOpts = program.opts();
    const verbose: boolean = globalOpts.verbose || false;

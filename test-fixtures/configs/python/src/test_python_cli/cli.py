...
class ConfigManager:
    ...
    def __init__(self, config_file: Optional[Path] = None):
        ...
        if config_file is None:
            config_dir = Path.home() / '.config' / 'testpy'
...
def cli(ctx, verbose, debug, config):
    """A test CLI for Python generation"""
...
def build_group(ctx):
    """Build something"""
    pass
@build_group.command('project')
@click.pass_obj
def build_project(ctx):
    """Build a project"""
    try:
        if hooks and hasattr(hooks, 'on_project'):
            kwargs = {            }
            hooks.on_project(ctx, **kwargs)
        else:
            logger.error(f"Hook 'on_project' not implemented in app_hooks.py")
            sys.exit(1)
    except Exception as e:
        handle_error(e, ctx.verbose)
@cli.command('serve')

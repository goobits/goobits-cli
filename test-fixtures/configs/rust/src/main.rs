// Test Rust CLI - Testing Rust generation
...
mod completion {
    ...
    pub fn generate_completion(app: &Command, shell: clap_complete::Shell) -> String {
        ...
        clap_complete::generate(shell, &mut app.clone(), "testrust", &mut buffer);
...
fn build_cli() -> Command {
    Command::new("testrust")
        .version("1.0.0")
        .about("A test CLI for Rust generation")
        .arg(
            Arg::new("verbose")
                .short('v')
                .long("verbose")
        ...
        .arg(
            ...
            Command::new("hello")
                .about("Say hello")        )        .subcommand(
            Command::new("build")
                .about("Build something")                .subcommand(
                    Command::new("project")
                        .about("Build a project")                )        )        .subcommand(
            Command::new("serve")
                .about("Start server")        )        .subcommand(
...
fn main() -> Result<()> {
    ...
    match matches.subcommand() {
        ...
        }        Some(("hello", sub_matches)) => {            debug!("Executing hello command");
            hooks::on_hello(sub_matches)
                .context("Failed to execute hello")?;        }        Some(("build", sub_matches)) => {            match sub_matches.subcommand() {                Some(("project", sub_sub_matches)) => {
                    debug!("Executing build project command");
                    hooks::on_build_project(sub_sub_matches)
                        .context("Failed to execute build project")?;
                }                _ => {
                    debug!("Executing build command");
                    hooks::on_build(sub_matches)
                        .context("Failed to execute build")?;
                }
            }        }        Some(("serve", sub_matches)) => {            debug!("Executing serve command");
            hooks::on_serve(sub_matches)
                .context("Failed to execute serve")?;        }        _ => {

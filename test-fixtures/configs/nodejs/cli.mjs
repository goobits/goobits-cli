...
class ConfigManager {
    ...
    _getDefaultConfigPath() {
        const configDir = join(homedir(), '.config', 'testnode');
    ...
    loadConfig() {
        ...
        for (const key of Object.keys(this.config)) {
            const envKey = `TESTNODE_${key.toUpperCase()}`;
...
program.name('testnode')
program.description('A test CLI for Node.js generation')
...
program
    .command('hello')
    .description('Say hello')    .action(async (...args) => {
        try {
...
const buildCommand = program
    .command('build')
    .description('Build something')    .action(async (...args) => {
        try {
            // Load hooks if not already loaded
            if (!hooks) {
                await loadHooks();
...
program
    .command('serve')
    .description('Start server')    .action(async (...args) => {
        try {
            // Load hooks if not already loaded
            if (!hooks) {
                await loadHooks();

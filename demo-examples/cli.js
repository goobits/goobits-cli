 ...
 * Demo Node.js CLI CLI - Node.js Implementation
 */
...
async function builtinUpgradeCommand(checkOnly = false, pre = false, version = null, dryRun = false) {
    if (checkOnly) {
        console.log(`Checking for updates to Demo Node.js CLI...`);
        console.log('Update check not yet implemented. Run without --check to upgrade.');
        return;
    }

    if (dryRun) {
        console.log('Dry run - would execute: npm update -g demo-nodejs-cli');
        return;
    }
    ...
    const searchPaths = [
        ...
        path.join(__dirname, '../setup.sh'),
        path.join(process.env.HOME, '.local', 'share', 'demo-nodejs-cli', 'setup.sh')
    ];
    ...
    if (!setupScript) {
        ...
        console.log(`Enhanced setup script not found. Using basic upgrade for Demo Node.js CLI...`);
        
        const packageName = 'demo-nodejs-cli';
        const npmName = 'demo-nodejs-cli';
        
        try {
            const cmd = process.platform === 'win32' ? 'npm.cmd' : 'npm';
            execSync(`${cmd} update -g ${npmName}`, { stdio: 'inherit' });
            console.log(`✅ Demo Node.js CLI upgraded successfully!`);
            console.log(`Run 'demo_js --version' to verify the new version.`);
        } catch (e) {
...
async function loadPlugins(program) {
    const pluginDirs = [
        path.join(process.env.HOME || process.env.USERPROFILE, '.config', 'goobits', 'demo_js', 'plugins'),
...
program
    .name('demo_js')
    .version(getVersion())
    .description(`demo_js v${getVersion()} - Node.js CLI demonstration`)
...
const greetCmd = program
    ...
    .option('-s, --style <value>', 'Greeting style', 'casual')
...
completionCmd
    ...
    .action((shell) => {
        ...
        console.log(`   demo_js completion generate ${shell} > completion.${shell}`);

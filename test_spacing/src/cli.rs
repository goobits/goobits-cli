...
mod completion {
    ...
    pub fn generate_completion(app: &Command, shell: clap_complete::Shell) -> String {
        ...
        clap_complete::generate(shell, &mut app.clone(), "spacingtest", &mut buffer);
...
fn build_cli() -> Command {
    Command::new("spacingtest")
        .version("1.0.0")
        .about("None")        .subcommand(
            Command::new("hello")
                .about("Say hello with spacing")                .arg(
                    Arg::new("name")
                        .help("Name to greet")
                        .required(false)                        .default_value("None")                        .index(1)
                )                .arg(
                    Arg::new("verbose")                        .short('v')                        .long("verbose")
                        .help("Verbose output")                        .value_name("VALUE")                )        )}
...
fn run_cli() -> Result<()> {
    ...
    match matches.subcommand() {
        Some(("completions", sub_matches)) => {
            ...
            cli_hooks::on_hello(sub_matches)
                .context("Failed to execute hello")?;        }        _ => {

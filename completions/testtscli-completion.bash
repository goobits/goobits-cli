#!/bin/bash
# Bash completion script for TestTSCLI (testtscli)
# Auto-generated by goobits-cli

_testtscli_completions() {
    local cur prev words cword
    _init_completion || return

    # Get the current CLI binary path
    local cli_bin="testtscli"
    ...
    case "${prev}" in
        testtscli)
            # Top level commands
            local commands="hello build upgrade plugin help"
            ...
            if [[ -n "$in_subcommand" ]]; then
                ...
                case "$in_subcommand" in
                    hello)
                        ;;
                    build)
                        local cmd_options="--watch --help -h"
...
complete -F _testtscli_completions testtscli
...
if [[ -n "${NPM_CONFIG_PREFIX:-}" ]]; then
    complete -F _testtscli_completions "${NPM_CONFIG_PREFIX}/bin/testtscli"
fi

# Support for npx execution
complete -F _testtscli_completions "npx testtscli"
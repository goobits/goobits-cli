#!/bin/bash
# Enhanced TypeScript setup script for Complex CLI Test Tool
...
print_tree_node() {
    ...
    case $status in
        "pending")
            echo -e "${prefix}${YELLOW}⏳${NC} $message"
            ;;
        "running")
            echo -e "${prefix}${BLUE}🔄${NC} $message"
            ;;
        "success")
            echo -e "${prefix}${GREEN}✅${NC} $message"
            ;;
        "error")
            echo -e "${prefix}${RED}❌${NC} $message"
            ;;
        "skip")
            echo -e "${prefix}${CYAN}⏭️${NC} $message (skipped)"
            ;;
        "warning")
            echo -e "${prefix}${YELLOW}⚠️${NC} $message"
            ;;
        *)
...
is_development() {
    [ -f "package.json" ] && [ -f "tsconfig.json" ] && [ -d "src" ]
}
...
check_npm_version() {
    if ! command -v npm &> /dev/null; then
        print_tree_node 2 false "error" "npm is not installed"
        log_error "npm is required. Please install npm."
        exit 1
    fi
    
    local npm_version=$(npm --version)
    print_tree_node 2 false "success" "npm version $npm_version detected"
}

# Check TypeScript compiler
check_typescript() {
    local needs_typescript=false
    
    # Check if TypeScript is installed locally
    if [ -f "node_modules/.bin/tsc" ]; then
        local tsc_version=$(./node_modules/.bin/tsc --version | grep -o '[0-9]\+\.[0-9]\+\.[0-9]\+')
        print_tree_node 2 false "success" "TypeScript compiler v$tsc_version (local)"
    elif command -v tsc &> /dev/null; then
        local tsc_version=$(tsc --version | grep -o '[0-9]\+\.[0-9]\+\.[0-9]\+')
        print_tree_node 2 false "success" "TypeScript compiler v$tsc_version (global)"
    else
        print_tree_node 2 false "warning" "TypeScript compiler not found"
        needs_typescript=true
    fi
    
    if [ "$needs_typescript" = true ]; then
        print_tree_node 3 false "running" "Installing TypeScript locally"
        npm install --save-dev typescript || {
            print_tree_node 3 false "error" "Failed to install TypeScript"
            exit 1
        }
        print_tree_node 3 false "success" "TypeScript installed"
    fi
}
...
install_system_dependencies() {
    ...
    local apt_packages=("build-essential" "git")
    
    if [ ${#apt_packages[@]} -gt 0 ]; then
        print_tree_node 2 false "running" "Installing system dependencies"
        
        if command -v apt-get &> /dev/null; then
            for package in "${apt_packages[@]}"; do
                if ! dpkg -l | grep -q "^ii  $package "; then
                    print_tree_node 3 false "running" "Installing $package"
                    sudo apt-get update -qq
                    sudo apt-get install -y "$package" || {
                        print_tree_node 3 false "error" "Failed to install $package"
                        exit 1
                    }
                    print_tree_node 3 false "success" "Installed $package"
                else
                    print_tree_node 3 false "skip" "$package already installed"
                fi
            done
        else
            print_tree_node 3 false "warning" "apt-get not available, skipping system packages"
        fi
    fi
...
install_dependencies() {
    ...
    local npm_packages=("prettier" "eslint")
    
    if [ ${#npm_packages[@]} -gt 0 ]; then
        print_tree_node 3 false "running" "Installing additional npm packages"
        for package in "${npm_packages[@]}"; do
            print_tree_node 4 false "running" "Installing $package"
            npm install -g "$package" || {
                print_tree_node 4 false "warning" "Failed to install $package globally"
            }
        done
    fi
...
build_typescript() {
    if is_development; then
        print_tree_node 2 false "running" "Building TypeScript project"
        
        # Check if we have a build script
        if npm run build --silent 2>/dev/null; then
            print_tree_node 3 false "success" "TypeScript build completed using npm script"
        elif [ -f "tsconfig.json" ]; then
            # Use tsc directly
            if [ -f "node_modules/.bin/tsc" ]; then
                ./node_modules/.bin/tsc || {
                    print_tree_node 3 false "error" "TypeScript compilation failed"
                    exit 1
                }
            elif command -v tsc &> /dev/null; then
                tsc || {
                ...
                print_tree_node 3 false "error" "TypeScript compiler not found"
                exit 1
            fi
            print_tree_node 3 false "success" "TypeScript compilation completed"
        else
            print_tree_node 3 false "warning" "No tsconfig.json found, skipping TypeScript build"
        fi
    else
        print_tree_node 2 false "skip" "Not in development mode, skipping TypeScript build"
    fi
}
...
setup_config_dir() {
    ...
    case "$OSTYPE" in
        darwin*)
            config_dir="$HOME/Library/Application Support/complex-cli-test"
            ;;
        msys*|win32*)
            config_dir="${APPDATA:-$HOME}/complex-cli-test"
            ;;
        *)
            config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/complex-cli-test"
            ;;
    esac
...
run_postinstall() {
    if [ -f "scripts/postinstall.ts" ]; then
        print_tree_node 2 false "running" "Running post-install configuration (TypeScript)"
        
        # Check if compiled version exists
        if [ -f "dist/scripts/postinstall.js" ]; then
            node dist/scripts/postinstall.js || {
                print_tree_node 3 false "warning" "Post-install script encountered issues"
            }
        elif command -v ts-node &> /dev/null; then
            ts-node scripts/postinstall.ts || {
                print_tree_node 3 false "warning" "Post-install script encountered issues"
            }
        else
            print_tree_node 3 false "warning" "Cannot run TypeScript post-install script (ts-node not available)"
        fi
        print_tree_node 3 false "success" "Post-install configuration completed"
    elif [ -f "scripts/postinstall.js" ]; then
...
uninstall() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/complex-cli-test"
        if [ -d "$config_dir" ]; then
...
show_help() {
    echo ""
    echo -e "${BLUE}Complex CLI Test Tool TypeScript Setup Script${NC}"
    echo "====================================="
    echo ""
    ...
    echo "  $0 install --dev       # Development installation with TypeScript build"
...
install() {
    ...
    # Task 2: TypeScript
    ((current_task++))
    show_progress $current_task $total_tasks "Checking TypeScript"
    echo ""
    print_tree_node 1 false "running" "Checking TypeScript compiler"
    check_typescript
    
    # Task 3: System dependencies
    ((current_task++))
    ...
    # Task 5: TypeScript build
    ((current_task++))
    show_progress $current_task $total_tasks "Building TypeScript"
    echo ""
    print_tree_node 1 false "running" "Building TypeScript project"
    build_typescript
    ...
    if is_development; then
        ...
        # Make both TS and compiled JS files executable
        if [ -f "dist/cli.js" ]; then
            chmod +x dist/cli.js
            print_tree_node 2 false "success" "Compiled CLI made executable"
        fi
        if [ -f "cli.ts" ]; then
            chmod +x cli.ts
            print_tree_node 2 false "success" "TypeScript CLI made executable"
        fi
        if [ -f "cli.js" ]; then
            chmod +x cli.js
            print_tree_node 2 false "success" "JavaScript CLI made executable"
        fi
    else
        ((current_task++))
    ...
    if [ "$dev_mode" = true ]; then
        ...
        echo "  • CLI is linked globally as: $COMMAND_NAME"
        echo "  • TypeScript files are compiled automatically"
        echo "  • Changes to source code will be reflected after rebuilding"
    elif [ "$global_mode" = true ]; then
        echo -e "${GREEN}Global installation completed:${NC}"
        echo "  • Use anywhere: $COMMAND_NAME"
    else
        echo -e "${GREEN}Local installation completed:${NC}"
        echo "  • Run locally: ./dist/cli.js (if built) or npx ts-node cli.ts"
        echo "  • Or install globally: npm install -g ."
    fi
    
    echo ""
    echo -e "${BLUE}Next steps:${NC}"
    echo "  • Run '$COMMAND_NAME --help' for usage information"
    echo "  • Check '$COMMAND_NAME config path' for configuration location"
    echo "  • Visit our documentation for more details"
    echo ""
}

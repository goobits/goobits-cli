 ...
 * Complex CLI Test Tool CLI - TypeScript Implementation
 */
...
async function builtinUpgradeCommand(checkOnly: boolean = false, pre: boolean = false, version: string | null = null, dryRun: boolean = false): Promise<void> {
    ...
    const searchPaths = [
        ...
        path.join(__dirname, '../setup.sh'),
        path.join(process.env.HOME || process.env.USERPROFILE || '', '.local', 'share', 'complex-cli-test', 'setup.sh')
    ];
...
const buildCmd = program
    ...
    .action(async (target: string, source: string, options: BuildOptions, command: Command) => {
        try {
...
testCmd
    ...
    .action(async (options: TestUnitOptions, command: Command) => {
        try {
            ...
            const exitCode = handleCLIError(error as Error, options.debug || (command.parent?.parent?.opts() as GlobalOptions)?.debug);
            process.exit(exitCode);
        }
    });
...
testCmd
    ...
    .action(async (service: string, options: TestIntegrationOptions, command: Command) => {
        try {
...
testCmd
    ...
    .action(async (options: TestPerformanceOptions, command: Command) => {
        try {
...
configCmd
    ...
    .action(async (key: string, options: ConfigGetOptions, command: Command) => {
        try {
...
configCmd
    ...
    .action(async (key: string, value: string, options: ConfigSetOptions, command: Command) => {
        try {

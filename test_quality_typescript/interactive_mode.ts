#!/usr/bin/env node
/**
 * Enhanced TypeScript Interactive Mode for Complex CLI Test Tool
...
class ComplexclitesttoolInteractive {
    ...
    private loadCommandHistory(): void {
        const historyFile = path.join(process.cwd(), '.complex-cli_history');
        try {
    ...
    start(): void {
        console.log('🚀 Welcome to Complex CLI Test Tool interactive mode');
    ...
    private async handleBuild(args: string[]): Promise<void> {
        const hookName = 'onBuild' as keyof HookRegistry;
        const hook = hooks[hookName];
        
        if (typeof hook === 'function') {
            try {
                // Parse arguments with type validation
                const parsedContext = await this.parseCommandContext('build', args);
                ...
                if (hasTypeScriptCompiler && this.typeChecker) {
                    const suggestions = this.typeChecker.getSuggestions(error as Error, 'build', args);
                    if (suggestions.length > 0) {
                        console.log('💡 Suggestions:');
                        suggestions.forEach(suggestion => console.log(`   • ${suggestion}`));
                    }
    ...
    private async handleConfig(args: string[]): Promise<void> {
        const hookName = 'onConfig' as keyof HookRegistry;
        const hook = hooks[hookName];
        
        if (typeof hook === 'function') {
            try {
                // Parse arguments with type validation
                const parsedContext = await this.parseCommandContext('config', args);
                await (hook as Function)(parsedContext);
            } catch (error) {
                console.error(`❌ Error in ${hookName}:`, (error as Error).message);
                ...
                // Enhanced error handling
                if (hasTypeScriptCompiler && this.typeChecker) {
                    const suggestions = this.typeChecker.getSuggestions(error as Error, 'config', args);
                    if (suggestions.length > 0) {
                        console.log('💡 Suggestions:');
                        suggestions.forEach(suggestion => console.log(`   • ${suggestion}`));
                    }
...
class EnhancedCompletionProvider implements CompletionProvider {
    ...
    getOptionCompletions(command: string, text: string): string[] {
        ...
        if (command === 'build') {
            ...
            options.push('-v');
            options.push('--output-format');
            options.push('-f');
            options.push('--parallel-jobs');
            options.push('-j');
    ...
    getValueCompletions(command: string, option: string, text: string): string[] {
        ...
        if (command === 'build') {
            if (option === '--output-format' || option === '-f') {
...
export function runInteractive(): void {
    const interactive = new ComplexclitesttoolInteractive();
    interactive.start();
}

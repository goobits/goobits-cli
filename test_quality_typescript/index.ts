...
const runtimeConfig: Partial<ConfigSchema> = {
  package_name: 'complex-cli-test',
  command_name: 'complex-cli',
  display_name: 'Complex CLI Test Tool',
  description: 'A complex CLI tool for testing code generation quality',
  version: '1.0.0',
};
...
async function builtinUpgradeCommand(options: UpgradeOptions): Promise<void> {
  ...
  const packageName: string = 'complex-cli-test';
  const displayName: string = 'Complex CLI Test Tool';
...
export async function cli(): Promise<void> {
  ...
  
  {
    ...
    cmd.action(async (target: unknown, source: unknown, options: Record<string, unknown>, command: Command) => {
  ...
  "prod"
    ...
    cmd.action(async (environment: unknown, options: Record<string, unknown>, command: Command) => {
      
      // Standard command - use hook pattern
      const hookName: string = 'onDeploy';
      if (appHooks && appHooks[hookName]) {
        const hookFunc = appHooks[hookName];
  ...
  
  {
    ...
    
    {
      ...
      subCmd.action(async (options: any, command: Command) => {
        const hookName: string = 'onTestUnit';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
    ...
    
    {
      ...
      subCmd.action(async (service: any, options: any, command: Command) => {
        const hookName: string = 'onTestIntegration';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
    ...
    
    {
      ...
      subCmd.action(async (options: any, command: Command) => {
        const hookName: string = 'onTestPerformance';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
  ...
  
  {
    ...
    
    {
      ...
      subCmd.action(async (key: any, options: any, command: Command) => {
        const hookName: string = 'onConfigGet';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
    ...
    
    {
      ...
      subCmd.action(async (key: any, value: any, options: any, command: Command) => {
        const hookName: string = 'onConfigSet';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
    ...
    
    {
      ...
      subCmd.action(async (options: any, command: Command) => {
        const hookName: string = 'onConfigList';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
  ...
  const testCmd = program.commands.find((c: Command) => c.name() === 'test');
  if (testCmd) {
  ...
  const deployCmd = program.commands.find((c: Command) => c.name() === 'deploy');
  if (deployCmd) {
  ...
  const configCmd = program.commands.find((c: Command) => c.name() === 'config');
  if (configCmd) {

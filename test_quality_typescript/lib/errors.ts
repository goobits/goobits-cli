/**
 * Error handling implementation for Complex CLI Test Tool
...
export class DefaultErrorManager implements ErrorManager {
  ...
  async handle<T extends CLIError>(error: T, context?: ErrorContext): Promise<void> {
    const handler = this.handlers.get(error.code);
    if (handler) {
      await handler(error, context || {});
    } else {
      // Default error handling
      const formatted = this.format(error, process.env.DEBUG !== undefined);
      
      if (formatted.developerMessage && process.env.DEBUG) {
...
export function handleError(error: CLIError, context?: ErrorContext): never {
  errorManager.handle(error, context);
  const formatted = errorManager.format(error);

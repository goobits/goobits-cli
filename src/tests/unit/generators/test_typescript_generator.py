...
class TestTypeScriptGenerator:
    ...
    def test_typescript_cli_structure_validation(self):
        ...
        assert (
            len(ts_files) > 0
        ), f"No TypeScript files found in {list(output_files.keys())}"
        ...
        assert (
            "import" in cli_content or "require(" in cli_content
        ), f"No imports found: {cli_content[:200]}"
    ...
    def test_typescript_unicode_special_characters(self):
        ...
        try:
            ...
            ts_file = (
                "generated_index.ts"
                if "generated_index.ts" in output_files
                else "index.ts"
            )
    ...
    def test_typescript_complex_command_generation(self):
        ...
        ts_file = (
            "generated_index.ts" if "generated_index.ts" in output_files else "index.ts"
        )
        cli_content = output_files[ts_file]
        ...
        assert (
            "input_file" in cli_content or "inputFile" in cli_content
        )  # TypeScript might use camelCase

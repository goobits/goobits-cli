...
class TestNodeJSE2EWorkflows:
    ...
    def test_complete_nodejs_workflow(self):
        ...
        cli_schema = CLISchema(
            ...
            commands={
                "hello": CommandSchema(
                    ...
                    options=[
                        OptionSchema(
                            name="uppercase", desc="Uppercase output", type="flag"
                        )
                    ],
                )
            },
        ...
        config = create_test_goobits_config(
            "workflow-cli", cli_schema, language="nodejs"
        )
    ...
    def test_generated_cli_syntax_validation(self):
        ...
        config = create_test_goobits_config(
            "syntax-test", cli_schema, language="nodejs"
        )
        ...
        try:
            ...
            assert (
                result.returncode == 0
            ), f"Generated CLI has syntax errors: {result.stderr}"

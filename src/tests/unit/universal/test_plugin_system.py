...
class TestPluginRegistry:
    ...
    def test_search_plugins(self):
        ...
        info1 = PluginInfo(name='test-plugin', description='A test plugin', tags=['testing'])
        info2 = PluginInfo(name='other-plugin', description='Another plugin', tags=['utility'])
...
class TestPluginCLIIntegrator:
    ...
    async def test_integrate_plugins_with_cli(self):
        ...
        with patch.object(self.integrator, 'get_plugin_commands',
                         AsyncMock(return_value={'test-cmd': PluginInfo(
                             name='test-plugin',
                             provides_commands=['test-cmd']
                         )})):
            
            base_config = {'name': 'test-cli'}
            enhanced_config = await self.integrator.integrate_plugins_with_cli(base_config)

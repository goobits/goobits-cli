...
"""
        ...
        rendered = self.renderer.render_component(
            "test_handler", test_template, context
        )
...
class TestTypeScriptRendererFunctional:
    ...
    def test_interfaces_generation(self):
        ...
        options_interfaces = [
            iface for iface in interfaces if "Options" in iface["name"]
        ]
        assert len(options_interfaces) > 0

        # Check interface structure
        compile_options = next(
            (
                iface
                for iface in options_interfaces
                if "CompileOptions" in iface["name"]
            ),
            None,
        )
        if compile_options:
        ...
        test_ir = {
            ...
            "cli": {
                "root_command": {
                    "name": "test-cli",
                    "subcommands": [
                        {"name": "build-project", "hook_name": "on_build_project"}
                    ],
                }
            },
        }
...
"""
        ...
        assert (
            ": string" in rendered or ": number" in rendered or ": boolean" in rendered
        )
...
"""
        ...
        test_context = {"project": {"name": "Universal CLI", "version": "1.0.0"}}

        # Test Node.js rendering
        nodejs_context = nodejs_renderer.get_template_context(
            {"project": test_context["project"]}
        )
        nodejs_result = nodejs_renderer.render_component(
            "universal", universal_template, nodejs_context
        )
        ...
        typescript_context = typescript_renderer.get_template_context(
            {"project": test_context["project"]}
        )
        typescript_result = typescript_renderer.render_component(
            "universal", universal_template, typescript_context
        )

...
class TestUniversalLoggerTemplate:
    ...
    def logger_template(self):
        ...
        template_path = Path(__file__).parent.parent.parent.parent / "goobits_cli/universal/components/logger.j2"
    ...
    def test_typescript_logger_generation(self, logger_template):
        ...
        context = {
            'language': 'typescript',
            'project': {'name': 'test-typescript-app'}
        }
        ...
        assert 'export interface LogLevel' in rendered or 'export type LogLevel' in rendered
    ...
    def test_all_languages_have_consistent_features(self, logger_template):
        ...
        for language in languages:
            ...
            for feature_category, features in core_features.items():
                ...
                assert features_found >= 1, f"{language} missing features from {feature_category}: {features}"
    ...
    def test_template_rendering_performance(self, logger_template):
        ...
        assert avg_time_per_render < 0.01, f"Template rendering too slow: {avg_time_per_render*1000:.2f}ms per render"
...
class TestLoggerComponentIntegration:
    ...
    def test_logger_template_file_exists(self):
        ...
        assert template_path.is_file(), f"Logger template path is not a file: {template_path}"
    ...
    def test_logger_component_completeness(self):
        ...
        for language in ['python', 'nodejs', 'typescript', 'rust']:
            ...
            assert len(rendered) > 2000, f"{language} logger implementation seems incomplete ({len(rendered)} chars)"

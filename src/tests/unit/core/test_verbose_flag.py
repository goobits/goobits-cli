...
class TestVerboseFlag:
    ...
    def test_self_hosted_goobits_includes_verbose_option(self):
        ...
        verbose_option = next((opt for opt in cli_options if opt.get('name') == 'verbose'), None)
        
        assert verbose_option is not None, "verbose option not found in goobits.yaml"
        assert verbose_option.get('short') == 'v', "verbose option should have short form -v"
        assert verbose_option.get('type') == 'flag', "verbose option should be a flag"
        assert "verbose" in verbose_option.get('desc', '').lower(), "verbose option should have appropriate description"
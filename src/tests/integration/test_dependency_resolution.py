...
"""
    ...
    def test_dependency_heavy_cli(self, language):
        ...
        if not all(runtime_results.values()):
            ...
            try:
                result = subprocess.run(
                    [command_name, "--help"], capture_output=True, text=True, timeout=10
                )
    ...
    def test_nodejs_dev_dependencies(self):
        ...
        temp_dir = self._create_temp_dir()
        config = TestConfigTemplates.complex_config(
            "typescript"
    ...
    def test_missing_dependencies_error_handling(self, language):
        ...
        try:
            if language == "python":
                ...
                if result.returncode != 0:
                    assert (
                        "Cannot find module" in result.stderr
                        or "MODULE_NOT_FOUND" in result.stderr
                        or "Cannot find package" in result.stderr
                    )
                ...
                if CargoManager.is_available():
                    try:
                        ...
                        if (
                            ...
                            pytest.skip(
                                f"Network connectivity issue: {e.stderr[:100]}..."
                            )
                        # Should fail with dependency errors if deps missing
                        assert (
                            "error" in e.stderr.lower() or "failed" in e.stderr.lower()
                        )
    ...
    def test_network_resilient_rust_dependencies(self):
        ...
        if network_available:
            ...
            try:
                ...
                assert (
                    result.returncode == 0
                ), f"Build failed with network: {result.stderr}"
            except subprocess.CalledProcessError as e:
    ...
    def test_dependency_conflict_resolution(self):
        ...
        try:
            ...
            test_env = TestEnvironmentManager.create_test_environment(
                temp_dir, "python"
            )
            ...
            assert all(
                runtime_results.values()
            ), f"Dependency conflicts caused runtime issues: {runtime_results}"
    ...
    def test_cross_language_isolation(self):
        ...
        if PipManager.is_available():
            python_command = self._generate_and_install_cli(
                python_config, python_temp_dir
            )
            python_env = TestEnvironmentManager.create_test_environment(
                python_temp_dir, "python"
            )
            ...
            assert all(
                python_results.values()
            ), f"Python CLI failed in scoped environment: {python_results}"
        else:
            pytest.skip("pip not available")

        # Install Node.js CLI in separate scoped environment
        if NpmManager.is_available():
            nodejs_command = self._generate_and_install_cli(
                nodejs_config, nodejs_temp_dir
            )
            nodejs_env = TestEnvironmentManager.create_test_environment(
                nodejs_temp_dir, "nodejs"
            )
            ...
            assert all(
                nodejs_results.values()
            ), f"Node.js CLI failed in scoped environment: {nodejs_results}"

            # Verify Python CLI still works (no cross-contamination)
            python_results_after = (
                DependencyValidator.check_runtime_dependencies_with_env(
                    python_command, python_env
                )
            )
            assert all(
                python_results_after.values()
            ), f"Python CLI contaminated by Node.js installation: {python_results_after}"
        else:
            pytest.skip("npm not available")
    ...
    def test_environment_cleanup_effectiveness(self):
        ...
        assert not (
            project_path / ".npm_prefix"
        ).exists(), "NPM prefix should be cleaned up"
        assert not (
    ...
    def test_real_cli_execution_validation(self):
        ...
        for language in [
            ...
            "nodejs",
            "rust",
        ]:  # Skip typescript for now (compilation complexity)
            ...
            try:
                ...
                ), f"{language} CLI help produced no output"
                ...
                if language == "python":
                    ...
                    if version_result.returncode != 0:
                        ...
                        assert (
                            "importerror" not in error_text
                        ), f"Python dependency missing: {version_result.stderr}"
                        assert (
                            "modulenotfounderror" not in error_text
                        ), f"Python module missing: {version_result.stderr}"
                    ...
                    if invalid_result.returncode != 0:
                        ...
                        assert (
                            "cannot find module" not in error_text
                        ), f"Node.js dependency missing: {invalid_result.stderr}"
                        assert (
                            "module_not_found" not in error_text
                        ), f"Node.js module missing: {invalid_result.stderr}"
                    ...
                    assert (
                        "usage" in help_text or "help" in help_text
                    ), f"Rust CLI help doesn't show clap formatting: {help_result.stdout}"
                ...
                if language == "rust" and "Could not connect" in str(e):
                    pytest.skip(
                        f"Skipping {language} due to network connectivity issues"
                    )
                else:

...
class PipManager:
    ...
    def install_editable(
        package_path: str, timeout: int = 120
    ...
    @staticmethod
    def install_editable_scoped(
        project_dir: str, venv_path: str = None
    ) -> subprocess.CompletedProcess:
...
class NpmManager:
    ...
    def install_dependencies(
        package_path: str, timeout: int = 300
    ) -> subprocess.CompletedProcess:
    ...
    def install_with_prefix(
        project_dir: str, prefix_path: str = None
    ) -> subprocess.CompletedProcess:
    ...
    @staticmethod
    def run_script(
        script_name: str, package_path: str, timeout: int = 120
    ) -> subprocess.CompletedProcess:
        """Run npm script."""
...
class CargoManager:
    ...
    @staticmethod
    def install_from_path_scoped(
        project_dir: str, cargo_home: str = None
    ) -> subprocess.CompletedProcess:
    ...
    @staticmethod
    def check(
        package_path: str, offline: bool = False, timeout: int = 120
    ) -> subprocess.CompletedProcess:
    ...
    def check_network_connectivity(timeout: int = 5) -> bool:
        ...
        try:
            ...
            response = urllib.request.urlopen(
                "https://index.crates.io/", timeout=timeout
            )
    ...
    def try_build_with_fallback(
        package_path: str, release: bool = False, timeout: int = 300
    ) -> subprocess.CompletedProcess:
        ...
        try:
            ...
            if "Could not connect" in e.stderr or "network" in e.stderr.lower():
                try:
                    return CargoManager.build(
                        package_path, release=release, offline=True, timeout=timeout
                    )
                except subprocess.CalledProcessError:
...
class PackageManagerRegistry:
    ...
    def get_available_managers(cls) -> Dict[str, bool]:
        ...
        return {name: manager.is_available() for name, manager in cls.managers.items()}
...
def validate_installation_environment() -> Dict[str, any]:
    ...
    rust_info = {"cargo_available": available_managers.get("cargo", False)}

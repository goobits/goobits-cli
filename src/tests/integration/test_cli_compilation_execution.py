...
class TestRustCLICompilation:
    """Test Rust CLI compilation and execution."""
    
    def test_rust_cli_compilation(self):
        """Test that generated Rust CLI compiles successfully."""
        if not check_tool_availability("cargo"):
            pytest.skip("cargo not available for Rust testing")
        
        config = create_realistic_config("rust")
        generator = RustGenerator()
        
        with tempfile.TemporaryDirectory() as temp_dir:
            # Generate CLI
            all_files = generator.generate_all_files(config, "test.yaml", "1.0.0")
            assert all_files, "Rust CLI generation failed"
            ...
            for filename, content in all_files.items():
                ...
                if filename.endswith('main.rs') or (filename.endswith('.rs') and 'main' in filename):
                    main_rs = file_path
                elif filename == 'Cargo.toml':
                    cargo_toml = file_path
            
            assert main_rs is not None, "No main.rs file found"
            assert cargo_toml is not None, "No Cargo.toml found"
            ...
            assert '[dependencies]' in cargo_content, "Cargo.toml should have [dependencies] section"
            assert 'clap' in cargo_content, "Should include clap dependency"
                ...
                if check_result.returncode == 0:
                    ...
                    if build_result.returncode != 0:
                        ...
                        if any(keyword in error_text for keyword in ['could not connect', 'network', 'index.crates.io']):
                            pytest.skip(f"Rust compilation skipped due to network issues: {build_result.stderr[:200]}")
                        else:
                            main_content = main_rs.read_text()
                            pytest.fail(f"Rust compilation failed: {build_result.stderr}\nMain.rs content:\n{main_content[:500]}...")
                    
                    # Verify binary was created
                    target_dir = Path(temp_dir) / "target" / "debug"
                    binary_files = list(target_dir.glob(f"*{config.command_name}*"))
                    if not binary_files:
                        binary_files = list(target_dir.glob("*"))  # Any binary
                    
                    assert len(binary_files) > 0, "No binary file generated from Rust compilation"
                    
                else:
                    # Check failed, examine error
                    error_text = check_result.stderr.lower()
                    if any(keyword in error_text for keyword in ['could not connect', 'network', 'index.crates.io']):
                        pytest.skip(f"Rust check skipped due to network issues: {check_result.stderr[:200]}")
                    else:
                        main_content = main_rs.read_text()
                        pytest.fail(f"Rust check failed: {check_result.stderr}\nMain.rs content:\n{main_content[:500]}...")
    ...
    def test_rust_cli_execution(self):
        """Test that compiled Rust CLI executes correctly."""
        if not check_tool_availability("cargo"):
            pytest.skip("cargo not available for Rust testing")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                pytest.skip("Rust build/execution timed out - likely network connectivity issue")
            
            print("✅ Rust CLI execution passed")

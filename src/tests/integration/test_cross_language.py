...
class EndToEndIntegrationTester:
        ...
        config_data = {
            ...
            "installation": {
                ...
                "extras": {
                    ...
                    "npm": ["chalk", "ora"] if language in ["nodejs", "typescript"] else [],
    ...
    def create_hook_implementations(self, temp_dir: str, language: str) -> Dict[str, str]:
...
'''
            ...
            hooks["src/hooks.rs"] = '''//! Hook implementations for testing CLI execution.
...
'''
    ...
    def test_end_to_end_integration_workflow(self) -> List[CLIExecutionResult]:
        ...
        for language in self.supported_languages:
            ...
            try:
                ...
                result.installation_success = execution_result.get("installation_success", False)
    ...
    def _test_language_specific_integration(self, language: str, temp_dir: str, all_files: Dict[str, str], 
        ...
        try:
            if language == "python":
                ...
                return self._test_typescript_cli_integration(temp_dir, all_files, config)
            elif language == "rust":
    ...
    def _test_python_cli_integration(self, temp_dir: str, all_files: Dict[str, str], 
                                   config: GoobitsConfigSchema) -> Dict[str, Any]:
                ...
                if len(found_commands) >= 2:
                    ...
                    try:
                        ...
                        if hello_result.returncode == 0:
                            result["warnings"].append("Hello command executed successfully")
    ...
    def _test_nodejs_cli_integration(self, temp_dir: str, all_files: Dict[str, str], 
                                   config: GoobitsConfigSchema) -> Dict[str, Any]:
        ...
        try:
            ...
            if package_json_path.exists():
                ...
                if npm_result.returncode == 0:
                    ...
                    result["warnings"].append(f"NPM install warning: {npm_result.stderr}")
    ...
    def _test_typescript_cli_integration(self, temp_dir: str, all_files: Dict[str, str], 
                                       config: GoobitsConfigSchema) -> Dict[str, Any]:
        ...
        try:
                ...
                if npm_result.returncode == 0:
                    ...
                    if (Path(temp_dir) / "tsconfig.json").exists():
                        ...
                        if compile_result.returncode == 0:
                            result["warnings"].append("TypeScript compilation successful")
                        else:
                            result["warnings"].append(f"TypeScript compilation warning: {compile_result.stderr}")
            ...
            # Test CLI help command
            help_result = subprocess.run(cmd, capture_output=True, text=True, cwd=temp_dir, timeout=30)
            ...
            if help_result.returncode == 0:
                ...
                if result["installation_success"]:
                    try:
                        hello_cmd = cmd[:-1] + ["hello", "World"]  # Replace --help with hello World
                        hello_result = subprocess.run(hello_cmd, capture_output=True, text=True, cwd=temp_dir, timeout=30)
    ...
    def _test_rust_cli_integration(self, temp_dir: str, all_files: Dict[str, str], 
                                 config: GoobitsConfigSchema) -> Dict[str, Any]:
            ...
            if (Path(temp_dir) / "Cargo.toml").exists():
                ...
                if build_result.returncode == 0:
                    ...
                    if cli_binary:
                        ...
                        if help_result.returncode == 0:
                            result["success"] = True
                            result["warnings"].append("Rust CLI help command executed successfully")
                            ...
                            result["error_message"] = f"Rust CLI help command failed: {help_result.stderr}"
                    else:
                        result["error_message"] = "Built Rust binary not found"
                else:
                    result["error_message"] = f"Rust build failed: {build_result.stderr}"
            else:
    ...
    def _find_cli_file(self, temp_dir: str, all_files: Dict[str, str], language: str) -> Optional[Path]:
    ...
    def test_error_handling_scenarios(self) -> List[CLIExecutionResult]:
        ...
        for language in self.supported_languages:
            ...
            try:
                ...
                result.success = graceful_count >= len(error_tests) * 0.5  # 50% threshold
    ...
    def _compile_integration_report(self, results: List[CLIExecutionResult]) -> Dict[str, Any]:
        ...
        test_groups = {}
        language_stats = {lang: {"passed": 0, "failed": 0, "warnings": 0, "hooks_working": 0} 
    ...
    def _assess_integration_health(self, results: List[CLIExecutionResult]) -> Dict[str, Any]:
        ...
        for language in self.supported_languages:
            ...
            if lang_results:
                ...
                assessment["hook_integration_status"][language] = lang_result.hook_executed
        
        # Calculate base score
        working_languages = sum(1 for lang_data in assessment["language_support"].values() 
        ...
        hooks_working = sum(1 for working in assessment["hook_integration_status"].values() if working)
        ...
        if working_languages < len(self.supported_languages):
            failed_languages = [lang for lang, data in assessment["language_support"].items() 
                              if not data["overall_working"]]
            assessment["recommendations"].append(f"Fix CLI generation for: {', '.join(failed_languages)}")
        
        if hooks_working < len(self.supported_languages):
            non_hook_languages = [lang for lang, working in assessment["hook_integration_status"].items() 
                                 if not working]
            assessment["recommendations"].append(f"Improve hook integration for: {', '.join(non_hook_languages)}")
        ...
        if not assessment["recommendations"]:
            assessment["recommendations"].append("System is functioning well - consider adding more advanced integration tests")
...
if PYTEST_AVAILABLE:
    class TestEndToEndIntegration:
        ...
        def test_comprehensive_integration_health(self):
            ...
            assert health_score >= 30.0, f"Integration health score too low: {health_score}%"
...
def run_standalone_tests():
    ...
    try:
        ...
        success_rate = len(successful_error_results) / len(error_results) if error_results else 0
        
        print(f"   Results: {len(successful_error_results)}/{len(error_results)} languages handling errors gracefully")
        ...
        all_tests_passed = (
            len(successful_results) >= 1 and
            success_rate >= 0.5 and
            health_score >= 30.0 and
            overall_success_rate >= 25.0 and
            hooks_executed >= 1
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--quick":
        ...
        try:
            ...
            print(f"   Installations Successful: {report['summary']['installations_successful']}")

...
class TestIntegrationWorkflows:
    ...
    def test_critical_installation_workflows(self, test_case):
        ...
        if not manager_class.is_available():
            pytest.skip(
                f"Package manager {primary_manager} not available for {language}"
            )
        ...
        assert result[
            "success"
        ], f"Integration test failed for {language}/{scenario}: {result.get('error', 'Unknown error')}"
        ...
        try:
            ...
            for i, language in enumerate(languages):
                ...
                config = TestConfigTemplates.minimal_config(
                    language, package_name=f"test-parallel-{language}-{i}"
                )
                ...
                if manager_class.is_available():
                    if language == "python":
                        install_result = self.runner._test_python_installation(
                            config, temp_dir
                        )
                    elif language == "nodejs":
                        install_result = self.runner._test_nodejs_installation(
                            config, temp_dir
                        )
                    ...
                    if install_result:
                        self.runner.track_installation(
                            primary_manager, config.package_name
                        )
            ...
            for language, result in results.items():
                if result["install_success"]:
                    ...
                    assert func_results[
                        "all_commands_work"
                    ], f"Parallel installation affected {language} CLI functionality"
    ...
    def test_cross_language_consistency(self):
        ...
        for language in ["python", "nodejs", "typescript", "rust"]:
            ...
            # Install CLI
            if language == "python":
                install_success = self.runner._test_python_installation(
                    config, temp_dir
                )
            elif language == "nodejs":
                install_success = self.runner._test_nodejs_installation(
                    config, temp_dir
                )
            elif language == "typescript":
                install_success = self.runner._test_typescript_installation(
                    config, temp_dir
                )
            elif language == "rust":
        ...
        if len(results) > 1:
            ...
            for language, outputs in results.items():
                ...
                assert outputs["--help"].get(
                    "has_output", False
                ), f"{language} CLI --help produces no output"

                # Version support is optional but should be consistent if present
                if "--version" in outputs and "error" not in outputs["--version"]:
                    assert outputs["--version"].get(
                        "has_output", False
                    ), f"{language} CLI --version produces no output"
    ...
    def test_installation_performance_benchmarks(self):
        ...
        for language, perf in performance_results.items():
            if perf["install_success"]:
                ...
                assert (
                    perf["generation_time"] < 10
                ), f"{language} generation took too long: {perf['generation_time']}s"

                # Total time should be reasonable (except Rust which compiles)
                max_time = (
                    300 if language == "rust" else 60
                )  # 5 minutes for Rust, 1 minute for others
                assert (
                    perf["total_time"] < max_time
                ), f"{language} installation took too long: {perf['total_time']}s"

...
class TestCLIGenerationIntegration:
    ...
    def test_cross_language_consistency(self, tmp_path):
        ...
        assert results["python"]["success"], f"Python generation failed: {results['python']['error']}"
        assert results["nodejs"]["success"], f"Node.js generation failed: {results['nodejs']['error']}"
        
        # TypeScript should also succeed
        assert results["typescript"]["success"], f"TypeScript generation failed: {results['typescript']['error']}"
        
        # Rust might fail if cargo is not available, but that's acceptable
        if shutil.which("cargo"):
            assert results["rust"]["success"], f"Rust generation failed: {results['rust']['error']}"
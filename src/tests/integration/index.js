...
async function builtinUpgradeCommand(options) {
  ...
  const displayName = 'NodeJSTestCLI';
...
function generateDynamicCompletion(shell) {
  ...
  switch (shell) {
    case 'bash':
      console.log(`# Basic bash completion for nodejstestcli
...
complete -F _nodejstestcli_completions nodejstestcli`);
    ...
    case 'zsh':
      console.log(`#compdef nodejstestcli
# Basic zsh completion for nodejstestcli
_nodejstestcli() {
    local commands=(
...
_nodejstestcli "$@"`);
    ...
    case 'fish':
      console.log(`# Basic fish completion for nodejstestcli
complete -c nodejstestcli -f
...
${commands.split(' ').map(cmd => 
  `complete -c nodejstestcli -n '__fish_use_subcommand' -a ${cmd} -d '${cmd} command'`
).join('\n')}`);
      break;
  }
}
...
'default'    );
    ...
    cmd.action(async (name, options, command) => {
      ...
      if (appHooks && appHooks[commandInstanceName]) {
        ...
        try {
          ...
          const commandError = error instanceof CLIError ? error : new CLIError(
            `Command 'deploy' execution failed: ${error.message}`,
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              ...
              `Failed to execute subcommand 'start'. ${error.message}`,
              1
            );
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              `Subcommand hook '${hookName}' failed: ${error.message}`,
              'SUBCOMMAND_EXECUTION_ERROR',
              `Failed to execute subcommand 'stop'. ${error.message}`,
              1
            );
            handleError(subcommandError);
          }
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              `Subcommand hook '${hookName}' failed: ${error.message}`,
              'SUBCOMMAND_EXECUTION_ERROR',
              `Failed to execute subcommand 'restart'. ${error.message}`,
              1
            );
            handleError(subcommandError);
          }
...
'up'      );
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              `Subcommand hook '${hookName}' failed: ${error.message}`,
              'SUBCOMMAND_EXECUTION_ERROR',
              `Failed to execute subcommand 'migrate'. ${error.message}`,
              1
            );
            handleError(subcommandError);
          }
    ...
    }
    {
      ...
      subCmd.action(async (dataset, options, command) => {
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              `Subcommand hook '${hookName}' failed: ${error.message}`,
              'SUBCOMMAND_EXECUTION_ERROR',
              `Failed to execute subcommand 'seed'. ${error.message}`,
              1
            );
            handleError(subcommandError);
          }
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              `Subcommand hook '${hookName}' failed: ${error.message}`,
              'SUBCOMMAND_EXECUTION_ERROR',
              `Failed to execute subcommand 'backup'. ${error.message}`,
              1
            );
            handleError(subcommandError);
          }
...
;
    ...
    cmd.action(async (pattern, options, command) => {
      // Standard command - use hook pattern
      ...
      if (appHooks && appHooks[hookName]) {
        ...
        try {
          ...
          const hookError = error instanceof CLIError ? error : new CLIError(
            `Hook function '${hookName}' failed: ${error.message}`,
            'HOOK_EXECUTION_ERROR',
            `Failed to execute command 'test'. ${error.message}`,
            1
          );
          handleError(hookError);
        }

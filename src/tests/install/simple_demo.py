...
def main():
    ...
    for language, managers in language_managers.items():
        available_for_lang = sum(1 for m in managers if available_managers.get(m, False))
        ...
        print(f"   {status} {language}: {available_for_lang}/{total_for_lang} package managers")
    ...
    print(f"   Package Manager Availability: {available_count}/{total_managers} ({readiness_score:.1%})")
    ...
    print("     cd src/tests/install")
    
    if available_managers.get('pip', False):
        print("     python3 -c \"from test_installation_workflows import *; print('Python tests ready')\"")
    
    if available_managers.get('npm', False):
        print("     python3 -c \"from package_manager_utils import NpmManager; print('Node.js tests ready')\"")

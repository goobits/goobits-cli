...
class FilePathCompletionProvider(CompletionProvider):
    ...
    def _sort_path_completions(self, path: str) -> tuple:
        """Sort key function for file path completions: directories first, then alphabetical."""
        return (not path.endswith("/"), path.lower())
...
class ConfigKeyProvider(CompletionProvider):
    ...
    def can_provide(self, context: CompletionContext) -> bool:
        ...
        if context.current_command in config_indicators or any(

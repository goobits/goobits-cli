...
class UniversalTemplateEngine:
    ...
    def _build_intermediate_representation(self, config, config_filename: str = "goobits.yaml") -> Dict[str, Any]:
        ...
        feature_requirements = self.analyze_feature_requirements(config, config_filename)
    ...
    def analyze_feature_requirements(self, config, config_filename: str = "goobits.yaml") -> Dict[str, Any]:
        """
    ...
    def _needs_rich_formatting(self, cli_config: Dict[str, Any], full_config: Dict[str, Any]) -> bool:
        ...
        if len(commands) <= 2:
            ...
            for cmd in commands.values():
                ...
                if any(marker in str(desc).lower() for marker in 
                      ['[bold]', '[italic]', '[green]', '[red]', '[yellow]', '[blue]', 
                       '[dim]', '[bright]', 'table', 'progress', 'spinner']):
                    return True
    ...
    def _needs_table_formatting(self, cli_config: Dict[str, Any]) -> bool:
        ...
        for cmd in commands.values():
            ...
            table_keywords = ['table', 'list', 'tabulate', 'grid', 'column', 'row', 'csv', 'export']
            ...
            for opt in options:
                ...
                opt_desc = str(opt_dict.get('desc', opt_dict.get('description', ''))).lower()
                
                if any(keyword in f"{opt_name} {opt_desc}" for keyword in table_keywords):
                    return True
    ...
    def _needs_progress_features(self, cli_config: Dict[str, Any]) -> bool:
        ...
        for cmd in commands.values():
            ...
            progress_keywords = ['progress', 'loading', 'spinner', 'wait', 'processing', 'download', 'upload', 'install', 'build']
    ...
    def _needs_color_support(self, cli_config: Dict[str, Any]) -> bool:
        ...
        for cmd in commands.values():
            ...
            for opt in options:
                ...
                if any(keyword in opt_name for keyword in ['color', 'no-color', 'theme', 'style']):
                    return True
        ...
        for cmd in commands.values():
            ...
            for items in [cmd_dict.get('args', []), cmd_dict.get('options', [])]:
                for item in items:
                    ...
                    if item_dict.get('type') in ['choice', 'file', 'path', 'json', 'object']:
                        return True
                    ...
                    if any(key in item_dict for key in ['min', 'max', 'pattern', 'validate']):
                        return True
    ...
    def _needs_file_operations(self, cli_config: Dict[str, Any]) -> bool:
        ...
        for cmd in commands.values():
            ...
            file_keywords = ['file', 'path', 'directory', 'folder', 'read', 'write', 'copy', 'move', 'delete', 'create', 'save', 'load', 'import', 'export']
    ...
    def _needs_commander_help_formatting(self, cli_config: Dict[str, Any]) -> bool:
        ...
        for cmd in commands.values():
            ...
            if any(keyword in desc.lower() for keyword in ['example', 'usage', 'note:', 'warning:']):
                return True

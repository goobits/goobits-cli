...
class VariableStore:
    ...
    def set_variable(self, name: str, raw_value: str, description: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            with self._lock:
                ...
                if (len(self.variables) >= self.max_variables and 
                ...
                logger.debug(f"Variable set: {name} = {parsed_value} ({var_type.value})")
                return True
    ...
    def get_completions_for_prefix(self, prefix: str) -> List[str]:
        ...
        """
        with self._lock:
            return [name for name in self.variables.keys() 

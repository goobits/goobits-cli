...
class RustCommandParser:
        ...
        try:
            ...
            while i < len(remaining_parts):
                ...
                if part.startswith('--'):
                    ...
                    option_def = self._find_option_definition(option_name, command_schema)
                        ...
                        if i + 1 < len(remaining_parts):
                            ...
                            if option_def:
                                ...
                                try:
                                    ...
                                    converted_value = self._convert_value(option_value, rust_type)

                                    options[option_name] = converted_value

                                except ValueError as e:

                                    return None, f"Invalid value for --{option_name}: {e}"
                    ...
                    option_def = self._find_short_option_definition(short_name, command_schema)
            ...
            required_count = sum(1 for arg in required_args if arg.get('required', True))

            

            if len(args) < required_count:

                return None, f"Command {command_name} requires at least {required_count} arguments, got {len(args)}"
            ...
            for i, arg_value in enumerate(args):
                ...
                if i < len(required_args):
                    ...
                    try:
                        ...
                        return None, f"Invalid value for argument {arg_def['name']}: {e}"
    ...
    def _find_option_definition(self, option_name: str, schema: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _find_short_option_definition(self, short: str, schema: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _convert_value(self, value: str, rust_type: str) -> Any:
        ...
        base_type = rust_type.split('<')[0].split('::')[-1]  # Handle Option<T> and paths
...
class RustCompletionProvider:
    ...
    def _complete_option_names(self, text: str, command_schema: Dict[str, Any]) -> List[str]:
    ...
    def _complete_option_value(self, option_name: str, text: str, command_schema: Dict[str, Any]) -> List[str]:
        ...
        if 'choices' in option_def:
            ...
            return [choice for choice in option_def['choices'] if choice.startswith(text)]
    ...
    def _complete_arguments(self, text: str, current_args: List[str], command_schema: Dict[str, Any]) -> List[str]:
        ...
        if arg_index < len(arguments):
            ...
            if 'choices' in arg_def:
                ...
                return [choice for choice in arg_def['choices'] if choice.startswith(text)]
...
class RustHistoryManager:
    ...
    def search_history(self, pattern: str, case_sensitive: bool = False) -> List[tuple[int, str]]:
...
class RustExpressionEvaluator:
    ...
    def evaluate_expression(self, expression: str, 
        ...
        try:
            ...
            if with_output:
                ...
                exec_result = self._execute_binary(compile_result['binary_path'], timeout_seconds)
    ...
    def _generate_program(self, expression: str) -> str:
        ...
        if expression.strip().endswith(';') or any(keyword in expression for keyword in ['let', 'if', 'for', 'while', 'match']):
    ...
    def validate_syntax(self, code: str) -> dict:
        ...
        try:
            ...
            return {
                ...
                'errors': "Syntax checking timed out"
...
'''
    ...
    def handle_search_history_command(self, args: List[str]):
        ...
        if case_sensitive:
            ...
            pattern = ' '.join([arg for arg in args if arg not in ['--case-sensitive', '-c']])
    ...
    def handle_crate_command(self, args: List[str]):
        ...
        if self.cargo_info and query.lower() in [dep.lower() for dep in self.cargo_info.dependencies]:

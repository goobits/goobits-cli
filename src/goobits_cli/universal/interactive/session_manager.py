...
class CommandEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "output_preview": (
                self.output_preview[:100] if self.output_preview else None
            ),
        }
...
class CommandHistory:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "CommandHistory":
        ...
        history.entries = [
            CommandEntry.from_dict(entry_data) for entry_data in data.get("entries", [])
        ]
        return history
...
class SessionManager:
    ...
    def __init__(
        ...
        try:
            ...
            self.session_dir = (
                Path(tempfile.gettempdir()) / "goobits_sessions" / cli_name
            )
            self.session_dir.mkdir(parents=True, exist_ok=True)

        logger.debug(
            f"Session manager initialized for {cli_name}, directory: {self.session_dir}"
        )
        ...
        try:
            with self._lock:
                ...
                session_metadata = SessionMetadata(
                    ...
                    total_duration_ms=int(
                        stats.get("avg_duration_ms", 0) * stats["total_commands"]
                    ),
                )
            ...
            if temp_file.exists():
                try:
                    ...
                    # Ignore file system errors when cleaning up temp files
                    pass
            return False

    def load_session(
        self, session_name: str
    ) -> Optional[tuple[CommandHistory, Dict[str, Any], Optional["VariableStore"]]]:
        """
        ...
        try:
            with self._lock:
                ...
                command_history = CommandHistory.from_dict(
                ...
                if "variable_store" in session_data:
                    try:
                        ...
                        variable_store = VariableStore.from_dict(
                            session_data["variable_store"]
                        )
                    except Exception as e:
                        logger.warning(
                            f"Failed to load variable store for session '{session_name}': {e}"
                        )
...
def create_session_manager(
    cli_name: str, config: Optional[Dict[str, Any]] = None
) -> SessionManager:
    """

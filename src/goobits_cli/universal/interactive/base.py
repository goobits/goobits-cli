        ...
        for command in root_command.get("subcommands", []):
            ...
            self.register_command(
                InteractiveCommand(
                    ...
                    handler=lambda args, cmd=command: self.handle_cli_command(
                        cmd, args
                    ),
            ...
            if cmd_name in self.commands:
                ...
                if command.arguments:
                    ...
                    for arg in command.arguments:
                        ...
                        print(
                            f"  {arg['name']}: {arg.get('description', '')} ({required})"
                        )
...
class InteractiveRenderer:
    ...
    def get_interactive_dependencies(language: str) -> Dict[str, List[str]]:
        ...
        dependencies = {
            "python": {"builtin": ["cmd", "shlex", "readline"], "external": []},
            "nodejs": {"builtin": ["readline"], "external": []},
            "typescript": {"builtin": ["readline"], "external": ["@types/node"]},
            "rust": {"builtin": [], "external": ["rustyline"]},
        }
...
class BasicREPL(InteractiveEngine):
    ...
    def __init__(
        self, cli_config: Dict[str, Any], smart_completion_enabled: bool = True
    ):
        """
    ...
    def _setup_readline(self):
        ...
        try:
            ...
            history_file = (
                f".{self.cli_config.get('root_command', {}).get('name', 'cli')}_history"
            )
            try:
        ...
        try:
            while True:
                try:
                    ...
                    complete_command, is_continuation = self.parse_multi_line_input(
                        line
                    )
...
class SessionREPL(BasicREPL):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.session_history = CommandHistory(
            max_entries=self.session_config.get("max_history", 1000)
        )
        ...
        self.register_command(
            InteractiveCommand(
                ...
                arguments=[
                    {
                        "name": "session_name",
                        "description": "Name of session to delete",
                        "required": True,
                    }
                ],
            )
    ...
    def handle_save_session(self, args: List[str]):
        ...
        success = self.session_manager.save_session(
            session_name, self.session_history, metadata
        )
    ...
    def handle_load_session(self, args: List[str]):
        ...
        if result:
            ...
            print(
                f"Session '{session_name}' loaded successfully ({command_count} commands)"
            )
        else:
    ...
    def handle_list_sessions(self, args: List[str]):
        ...
        print("\nAvailable sessions:")
        print(
            f"{'Name':<20} {'Commands':<10} {'Success Rate':<12} {'Last Modified':<20}"
        )
        print("-" * 65)
        ...
        for session in sessions:
            ...
            last_mod = datetime.fromtimestamp(session["last_modified"]).strftime(
                "%Y-%m-%d %H:%M"
            )
            success_rate = f"{session['success_rate']:.1%}"
            current = (
                " (current)" if session["name"] == self.current_session_name else ""
            )

            print(
                f"{session['name']:<20} {session['command_count']:<10} {success_rate:<12} {last_mod:<20}{current}"
            )
        print()
    ...
    def handle_delete_session(self, args: List[str]):
        ...
        try:
            response = (
                input(f"Delete session '{session_name}'? (y/N): ").strip().lower()
            )
...
class VariableREPL(SessionREPL):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.variable_substitution_enabled = self.variable_config.get(
            "substitution", True
        )
        ...
        self.register_command(
            InteractiveCommand(
                ...
                arguments=[
                    {
                        "name": "assignment",
                        "description": "Variable assignment (name=value)",
                        "required": True,
                    }
                ],
            )
    ...
    def handle_set_variable(self, args: List[str]):
        ...
        if success:
            ...
            if variable:
                ...
                display_value = TypeInferenceEngine.format_value_for_display(
                    variable.value, variable.var_type
                )
                print(
                    f"Variable set: {var_name} = {display_value} ({variable.var_type.value})"
                )
        else:
    ...
    def handle_list_variables(self, args: List[str]):
        ...
        for var in variables:
            ...
            print(
                f"{var.name:<{name_width}} {var.var_type.value:<{type_width}} {display_value}"
            )
    ...
    def handle_variable_stats(self, args: List[str]):
        ...
        print(
            f"  Usage: {stats['usage_percentage']:.1f}% of {stats['max_variables']} max"
        )
    ...
    def execute_command(self, line: str) -> bool:
        ...
        if (
            ...
            and self.variable_substitution_enabled
    ...
    def handle_save_session(self, args: List[str]):
        ...
        success = self.session_manager.save_session(
            session_name, self.session_history, metadata, variable_store
        )
    ...
    def handle_load_session(self, args: List[str]):
        ...
        if result:
            ...
            command_count = session_meta.get("command_count", 0)
            print(
                f"Session '{session_name}' loaded successfully ({command_count} commands){var_info}"
            )
        else:
            print(f"Failed to load session '{session_name}' or session not found")
...
class PipelineREPL(VariableREPL):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pipeline_templates_enabled = self.pipeline_config.get(
        ...
        self.register_command(
            InteractiveCommand(
                ...
                arguments=[
                    {
                        "name": "definition",
                        "description": "Pipeline definition (name { commands })",
                        "required": True,
                    }
                ],
            )
        ...
        self.register_command(
            InteractiveCommand(
                ...
                arguments=[
                    {
                        "name": "pipeline_name",
                        "description": "Name of pipeline template to run",
                        "required": True,
                    },
                    {
                        "name": "parameters",
                        "description": "Template parameters (key=value)",
                        "required": False,
                    },
                ],
            )
    ...
    def handle_pipeline_definition(self, args: List[str]):
        ...
        if not args:
            ...
            print(
                "       pipeline name(param1,param2) { command1 --arg $param1 | command2 }"
            )
            print("Examples:")
            print("  pipeline greet_users { list-users | greet --style casual }")
            print(
                "  pipeline process_data(format) { load-data | transform --format $format | save-output }"
            )
            return
        ...
        if not match:
            ...
            print(
                "Expected: pipeline_name { commands } or pipeline_name(params) { commands }"
            )
            return
        ...
        success = self.pipeline_processor.define_pipeline_template(
            pipeline_name, commands, parameters
        )

        if success:
            param_info = (
                f" with parameters: {', '.join(parameters)}" if parameters else ""
            )
            print(
                f"Pipeline template '{pipeline_name}' defined successfully{param_info}"
            )
        else:
    ...
    def handle_run_pipeline(self, args: List[str]):
        ...
        try:
            ...
            success, message = loop.run_until_complete(
                self.pipeline_processor.execute_pipeline_template(
                    pipeline_name, parameter_values
                )
            )
    ...
    def handle_list_pipelines(self, args: List[str]):
        ...
        print(
            f"{'Name':<20} {'Commands':<10} {'Parameters':<15} {'Usage':<8} {'Description':<30}"
        )
        print("-" * 85)

        for template in templates:
            params = (
                ", ".join(template["parameters"]) if template["parameters"] else "none"
            )
            if len(params) > 12:
                params = params[:9] + "..."
            ...
            print(
                f"{template['name']:<20} {template['command_count']:<10} {params:<15} {template['usage_count']:<8} {description:<30}"
            )
    ...
    def get_basic_completions(self, text: str) -> List[str]:
        ...
        if (
            ...
            matching_templates = [
                name for name in template_names if name.startswith(prefix)
            ]
...
def create_basic_repl(
    cli_config: Dict[str, Any], **kwargs
) -> Union[BasicREPL, SessionREPL, VariableREPL, PipelineREPL]:
    """
    ...
    if pipelines_enabled:
        ...
        return SessionREPL(
            cli_config, session_config, smart_completion_enabled=smart_completion
        )

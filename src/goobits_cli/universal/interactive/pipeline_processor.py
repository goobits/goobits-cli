...
class StreamData:
    ...
    def to_text(self) -> str:
        ...
        if self.format == DataFormat.TEXT:
            ...
            return json.dumps(self.content, indent=2) if isinstance(self.content, (dict, list)) else str(self.content)
        elif self.format == DataFormat.LINES:
            return '\n'.join(str(line) for line in self.content) if isinstance(self.content, list) else str(self.content)
        else:
            return str(self.content)
    ...
    def to_json(self) -> Any:
        ...
        if self.format == DataFormat.JSON:
            ...
            return list(self.content) if isinstance(self.content, (list, tuple)) else [str(self.content)]
        else:
            return self.content
...
class StreamProcessor:
    ...
    async def transform_data(self, data: StreamData, target_format: DataFormat) -> StreamData:
        """
        ...
        try:
            ...
            raise ValueError(f"Cannot transform data from {data.format.value} to {target_format.value}: {e}")
    
    def validate_data_flow(self, output_format: DataFormat, input_format: DataFormat) -> bool:
        """
        ...
        if input_format == DataFormat.JSON and output_format in [DataFormat.JSON, DataFormat.STRUCTURED]:
            return True
        
        # Lines format can accept text or lines
        if input_format == DataFormat.LINES and output_format in [DataFormat.TEXT, DataFormat.LINES]:
            return True
...
class PipelineProcessor:
    ...
    def __init__(self, cli_config: Dict[str, Any], variable_store=None, timeout: int = 60):
        """
        ...
        try:
            ...
            for part in pipeline_parts:
                ...
                try:
                    ...
                    commands.append(PipelineCommand(
                        name=cmd_name,
                        args=cmd_args,
                        original_line=part
                    ))
    ...
    def define_pipeline_template(self, name: str, definition: str, parameters: Optional[List[str]] = None) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Pipeline template '{name}' defined with {len(commands)} commands")
            return True
    ...
    async def execute_pipeline_template(self, name: str, parameter_values: Dict[str, str] = None) -> Tuple[bool, str]:
        """
            ...
            if template.parameters and parameter_values:
                for param in template.parameters:
                    if param in parameter_values:
                        ...
                        command_line = command_line.replace(f"${param}", parameter_values[param])
    ...
    async def execute_pipeline(self, line: str, timeout: Optional[int] = None) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            try:
                ...
                with self._pipeline_lock:
                    if execution_id in self.active_pipelines:
                        ...
                        self.active_pipelines[execution_id]['completed_at'] = time.time()
            ...
            with self._pipeline_lock:
                if execution_id in self.active_pipelines:
                    self.active_pipelines[execution_id]['status'] = PipelineStatus.FAILED
    ...
    async def _execute_pipeline_commands(self, commands: List[PipelineCommand], execution_id: str) -> Tuple[bool, str]:
        """
                ...
                if command.name == "list-users":
                    ...
                    if "--active" in command.args:
                        if current_data.format == DataFormat.LINES:
                            filtered = [user for user in current_data.content if user != "charlie"]  # Mock filter
                            current_data = StreamData(
                                content=filtered,
                                format=DataFormat.LINES,
                                metadata=current_data.metadata,
                            ...
                            output_lines.append(f"[{command.name}] Filtered to {len(filtered)} active users")
                    ...
                    if current_data.format == DataFormat.LINES:
                        greetings = [f"Hello {user} ({style})" for user in current_data.content]
                        current_data = StreamData(
                            content=greetings,
                            format=DataFormat.LINES,
                            metadata=current_data.metadata,
                        ...
                        output_lines.append(f"[{command.name}] Generated {len(greetings)} greetings")
                
                else:
                    # Generic command processing
                    output_lines.append(f"[{command.name}] Processed data: {command.args}")
            
            final_output = "\n".join(output_lines)
            if current_data.format == DataFormat.LINES and current_data.content:
                final_output += "\n\nFinal output:\n" + "\n".join(str(item) for item in current_data.content)
    ...
    def get_execution_statistics(self) -> Dict[str, Any]:
        ...
        with self._pipeline_lock:
            ...
            successful_executions = sum(1 for exec_info in self.execution_history if exec_info['success'])
            ...
            if self.pipeline_templates:
                most_used_template = max(self.pipeline_templates.values(), key=lambda t: t.usage_count).name
    ...
    def cleanup_execution_history(self, max_age_hours: int = 24):
        ...
        with self._pipeline_lock:
            ...
            logger.info(f"Cleaned up pipeline execution history older than {max_age_hours} hours")


def create_pipeline_processor(cli_config: Dict[str, Any], variable_store=None, **kwargs) -> Optional[PipelineProcessor]:
    """

...
class TemplateAnalyzer:
    ...
    def compare_templates_across_languages(
        self, template_base_name: str
    ) -> ComparisonResult:
        """
    ...
    def _calculate_similarity_score(
        self, templates: Dict[str, TemplateAnalysis]
    ) -> float:
        """
        ...
        for i in range(len(template_list)):
            for j in range(i + 1, len(template_list)):
                ...
                filter_similarity = (
                    common_filters / total_filters if total_filters > 0 else 1.0
                )
                ...
                block_similarity = (
                    common_blocks / total_blocks if total_blocks > 0 else 1.0
                )
    ...
    def _generate_recommendations(
        self,
        templates: Dict[str, TemplateAnalysis],
        common_vars: Set[str],
        common_filters: Set[str],
        common_blocks: Set[str],
    ) -> List[str]:
    ...
    def _generate_text_report(self) -> str:
        ...
        lines = ["# Template Analysis Report", "=" * 50, ""]
    ...
    def _generate_markdown_report(self) -> str:
        ...
        for language in sorted(languages):
            ...
            avg_complexity = (
                sum(t.complexity_score for t in lang_templates) / count
                if count > 0
                else 0
            )

            lines.append(
                f"| {language.capitalize()} | {count} | {avg_complexity:.2f} |"
            )
...
def analyze_templates_command(
    templates_dir: Path = typer.Argument(..., help="Path to templates directory"),
    ...
    compare: Optional[str] = typer.Option(
        None, "--compare", help="Compare templates with this base name across languages"
    ),
):
    """

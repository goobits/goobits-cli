...
class SessionSubprocessCache:
    ...
    def _generate_cache_key(self, cmd: List[str], **kwargs) -> str:
        ...
        env_context = {var: os.environ.get(var) for var in relevant_env_vars if var in os.environ}
        ...
        return hashlib.sha256(context_str.encode()).hexdigest()[:16]  # Use first 16 chars
    ...
    def _detect_operation_type(self, cmd: List[str]) -> str:
        ...
        # Package managers
        if command in ('pip', 'pipx', 'npm', 'yarn', 'pnpm', 'cargo', 'apt', 'brew', 'pacman'):
    ...
    def run_cached(
        ...
        **kwargs
        ...
        try:
            ...
            should_cache = (
                result.returncode == 0 or  # Success
                (operation_type == 'package_manager' and result.returncode in (1, 2)) or  # Common pkg mgr codes
                (operation_type == 'system_check' and result.returncode == 127)  # Command not found
            )
            ...
            if should_cache:
                ...
                logger.debug(f"Cached result for {operation_type} command (TTL: {ttl}s)")

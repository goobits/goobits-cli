...
class ParallelIOManager:
            ...
            try:
                ...
                if AIOFILES_AVAILABLE:
                    ...
                    content = await loop.run_in_executor(
                        self._executor, lambda: path.read_text(encoding="utf-8")
                    )
    ...
    async def _threaded_read_files(
    ...
    async def process_templates_parallel(
        self, templates: Dict[str, str], processor, context: Dict[str, Any]
    ) -> Dict[str, str]:
        """
    ...
    async def _process_template_async(
        self, template: str, processor, context: Dict[str, Any]
    ) -> str:
            ...
            try:
                ...
                if asyncio.iscoroutinefunction(processor):
                    ...
                    return await loop.run_in_executor(
                        self._executor, processor, template, context
                    )
            except Exception as e:
...
async def write_files_batch(
    files: List[Tuple[Path, str]], batch_size: int = 10
) -> bool:
    """
...
async def optimize_template_generation(
    template_files: List[Path], renderer, context: Dict[str, Any], output_dir: Path
) -> Dict[str, bool]:
    """
    ...
    return {output_files[i][0]: results[i] for i in range(len(output_files))}

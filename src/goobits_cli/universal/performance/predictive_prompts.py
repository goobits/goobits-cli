...
class PromptPredictor:
    ...
    def predict_next_prompts(
        self, current_prompt: str, context: Dict[str, Any], max_predictions: int = 3
        ...
        with self._lock:
            ...
            total_transitions = sum(
                count
                for (prev, _), count in self.transitions.items()
            ...
            for key, value in simple_context.items():
                ...
                if pattern_key in self.context_patterns:
                    total_context_weight = sum(
                        self.context_patterns[pattern_key].values()
                    )
                    if total_context_weight > 0:
                        for prompt_id, weight in self.context_patterns[
                            pattern_key
                        ].items():
...
class PredictivePromptManager:
    ...
    async def _prepare_prompt(
        self, prompt_config: PromptConfig, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if prompt_config.choices:
            if callable(prompt_config.choices):
                try:
                    ...
                    logger.warning(
                        f"Failed to pre-load choices for {prompt_config.id}: {e}"
                    )
                    prepared["choices"] = []
            else:
...
def get_prepared_prompt(
    prompt_id: str, context: Dict[str, Any]
) -> Optional[Dict[str, Any]]:
    """Get a pre-prepared prompt if available."""
    manager = get_prompt_manager()

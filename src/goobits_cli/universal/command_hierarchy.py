...
class CommandFlattener:
    ...
    def _extract_commands_recursive(self, commands_dict: Dict[str, Any], 
                                   parent_path: List[str], 
                                   flat_commands: List[FlatCommand]) -> None:
        """
        ...
        for cmd_name, cmd_data in commands_dict.items():
            ...
            if has_subcommands:
                ...
                if isinstance(subcommands_data, dict):
                    ...
                    print(f"Warning: Unexpected subcommands format for {current_path}: {type(subcommands_data)}")
...
class HierarchyBuilder:
    ...
    def _build_command_groups(self, groups: List[FlatCommand], 
                             leaves: List[FlatCommand]) -> List[CommandGroup]:
        """
    ...
    def _find_direct_subcommands(self, parent: FlatCommand, 
                                all_commands: List[FlatCommand]) -> List[CommandNode]:
        """
        ...
        for cmd in all_commands:
            ...
            if (len(cmd.path) == len(parent.path) + 1 and 
                cmd.path[:-1] == parent.path):
...
class HookNameResolver:
    ...
    def _discover_available_hooks(self) -> List[str]:
        ...
        return [name for name in dir(self.hooks_module) 

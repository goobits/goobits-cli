...
class PatternAnalyzer:
    ...
    def __init__(self, base_path: str = "/workspace/goobits-cli/src/goobits_cli/templates"):
    ...
    def _generate_report(self) -> Dict[str, Any]:
        ...
        report: Dict[str, Any] = {
            'summary': {
                ...
                'categories': {
                    cat: len(patterns) for cat, patterns in self.pattern_categories.items()
                }
            },
            'patterns': {},
            'categories': {},
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        print("-"*60)
        universal_patterns = [p for name, p in report['patterns'].items() if p['is_universal']]
        ...
        print("-"*60)
        specific_patterns = [p for name, p in report['patterns'].items() if not p['is_universal']]
    ...
    def save_report(self, report: Dict[str, Any], filename: str = "pattern_inventory.md"):
        ...
        with open(output_path, 'w') as f:
            ...
            f.write("## Summary\n\n")
            f.write(f"- **Total Patterns Found**: {report['summary']['total_patterns']}\n")
            f.write(f"- **Extractable Patterns**: {report['summary']['extractable_patterns']}\n")
            f.write(f"- **Languages Analyzed**: {report['summary']['languages_analyzed']}\n\n")
            ...
            for pattern in universal_patterns:
                ...
                f.write(f"- **Extractable**: {'✅ Yes' if pattern['extractable'] else '❌ No'}\n")
                if pattern['notes']:
            ...
            for agent, patterns in report['boundaries'].items():
                if agent.startswith('agent_'):
                    agent_name = agent.replace('_', ' ').replace('agent ', 'Agent ').title()

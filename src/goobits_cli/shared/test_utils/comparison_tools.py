...
class CrossLanguageComparator:
    ...
    def compare_cli_outputs(
        ...
        result.summary = f"Compared {len(languages)} languages for command: {' '.join(command)}"
        ...
        for section in help_sections:
            ...
            if languages_with_section:
                if len(languages_with_section) == len(outputs):
                    ...
                    missing_languages = set(outputs.keys()) - set(languages_with_section)
    ...
    def _compare_version_outputs(
        ...
        if len(unique_versions) == 1:
            result.add_similarity(f"All languages report same version: {list(unique_versions)[0]}")
        ...
        for indicator in error_indicators:
            ...
            if len(languages_with_indicator) > 0:
                if len(languages_with_indicator) == len(outputs):
                    result.add_similarity(f"All languages show error indicator: '{indicator}'")
                else:
                    missing_languages = set(outputs.keys()) - set(languages_with_indicator)
                    result.add_difference(
                        'error_indicators',
    ...
    def compare_file_structures(
        ...
        if len(set(file_counts.values())) == 1:
            result.add_similarity(f"All languages generate same number of files: {list(file_counts.values())[0]}")
        else:
            result.add_difference(
                'file_count',
        ...
        for expected_file in expected_files:
            ...
            if languages_with_file:
                if len(languages_with_file) == len(file_structures):
                    ...
                    missing_languages = set(file_structures.keys()) - set(languages_with_file)
                    result.add_difference(
                        'missing_files',

...
class ManifestUpdater:
    ...
    def update_package_json(self, 
        ...
        try:
            ...
            warnings = self._merge_nodejs_config(package_data, cli_name, cli_file, dependencies)
    ...
    def _merge_nodejs_config(self, 
                            package_data: Dict[str, Any],
                            cli_name: str, 
                            cli_file: str,
                            additional_deps: Optional[Dict[str, str]]) -> List[str]:
        """
        ...
        if current_type is None:
            ...
            import typer
            typer.echo(f"⚠️  Warning: package.json has type='{current_type}' but generated CLI requires ES6 modules.")
            typer.echo("   This will change your package.json type to 'module' which may affect existing CommonJS code.")
            
            proceed = typer.confirm("   Continue and change type to 'module'?")
            if proceed:
                package_data["type"] = "module"
                warnings.append(f"Changed package.json type from '{current_type}' to 'module'")
            else:
                # User declined - this should be treated as an error to abort manifest update
                raise ValueError(f"User declined to change package.json type from '{current_type}' to 'module'")

...
class V3_0_0_Migration(Migration):
        ...
        if isinstance(value, dict):
            ...
            if key == "subcommands" and isinstance(value, dict):
                ...
                for sub_key, sub_value in value.items():
                    migrated_subcommands[sub_key] = self._migrate_value(sub_value, sub_key)
    ...
    def _convert_subcommands_array_to_object(self, subcommands_array: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def get_changes(self, original: Dict[str, Any], migrated: Dict[str, Any]) -> List[str]:
        ...
        def find_subcommand_changes(orig_data, migr_data, path=""):
            if isinstance(orig_data, dict) and isinstance(migr_data, dict):
                for key in orig_data:
                    if key == "subcommands":
                        if isinstance(orig_data[key], list) and isinstance(migr_data[key], dict):
                            location = f"{path}.{key}" if path else key
                            changes.append(f"Converted subcommands array → object at {location}")
                    elif key in migr_data:
                        new_path = f"{path}.{key}" if path else key
                        find_subcommand_changes(orig_data[key], migr_data[key], new_path)

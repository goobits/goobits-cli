...
def load_hooks():
    ...
    try:
        ...
        logger.warning(
            "No cli_hooks.py found. Please create one with your command implementations."
        )
...
def build(ctx, config_path, output_dir, output, backup):
    ...
    try:
        if hooks and hasattr(hooks, "on_build"):
            ...
            logger.error("Hook 'on_build' not implemented in cli_hooks.py")
            sys.exit(1)
    except Exception as e:
...
def init(ctx, project_name, template, force):
    ...
    try:
        if hooks and hasattr(hooks, "on_init"):
            ...
            logger.error("Hook 'on_init' not implemented in cli_hooks.py")
            sys.exit(1)
    except Exception as e:
        handle_error(e, ctx.verbose)
...
@click.argument("config_path", type=click.STRING)
@click.option(
    "--verbose", "-v", default=None, help="📋 Show detailed validation information"
)
@click.pass_obj
def validate(ctx, config_path, verbose):
    ...
    try:
        if hooks and hasattr(hooks, "on_validate"):
            ...
            logger.error("Hook 'on_validate' not implemented in cli_hooks.py")
            sys.exit(1)
    except Exception as e:
        handle_error(e, ctx.verbose)
...
def migrate(ctx, path, backup, dry_run, pattern):
    ...
    try:
        if hooks and hasattr(hooks, "on_migrate"):
            ...
            logger.error("Hook 'on_migrate' not implemented in cli_hooks.py")
            sys.exit(1)
    except Exception as e:
        handle_error(e, ctx.verbose)

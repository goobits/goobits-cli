...
║  Generated on: 2025-08-25 23:32:35            ║
║                                                                              ║
...
def validate(ctx, config_path, verbose):
    ...
    try:
        if hooks is None:
            ...
            raise ConfigError(
                error_msg,
                suggestion=f"Create app_hooks.py with function: def on_validate(...): pass"
            )
        
        hook_name = 'on_validate'
        if not hasattr(hooks, hook_name):
            available_hooks = [name for name in dir(hooks) if not name.startswith('_') and callable(getattr(hooks, name))]
            raise ConfigError(
                f"Hook function '{hook_name}' not found in app_hooks.py",
...
'verbose': verbose}
        ...
        # Handle return codes
        if isinstance(result, int) and result != 0:
            click.echo(f"Command 'validate' failed with exit code {result}", err=True)
            sys.exit(result)
...
@click.option(
    "--dry-run",
    help="👁️ Show changes without applying them",
    is_flag=True)
@click.option(
    "--pattern",
...
def migrate(ctx, path, backup, dry_run, pattern):
    ...
    try:
        if hooks is None:
            ...
            raise ConfigError(
                error_msg,
                suggestion=f"Create app_hooks.py with function: def on_migrate(...): pass"
            )
        
        hook_name = 'on_migrate'
        if not hasattr(hooks, hook_name):
            available_hooks = [name for name in dir(hooks) if not name.startswith('_') and callable(getattr(hooks, name))]
            raise ConfigError(
                f"Hook function '{hook_name}' not found in app_hooks.py",
...
'pattern': pattern}
        ...
        # Add positional arguments as kwargs if function expects them
        if 'path' in expected_params or accepts_kwargs:
        ...
        # Handle return codes
        if isinstance(result, int) and result != 0:
            click.echo(f"Command 'migrate' failed with exit code {result}", err=True)
            sys.exit(result)

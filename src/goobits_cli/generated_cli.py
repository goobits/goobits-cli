...
║  Generated on: 2025-08-25 20:02:02            ║
║                                                                              ║
...
class HookError(CLIError):
    ...
    def __init__(self, message: str, hook_name: str = None):
        ...
        super().__init__(
            ...
            exit_code=3,
            suggestion=(
                f"Check the '{hook_name}' function in your app_hooks.py file"
                if hook_name
                else None
            ),
        )
...
def _path_to_import_path(
    file_path: str, package_name: str = "goobits_cli"
) -> tuple[str, str]:
...
def _find_and_import_hooks():
        ...
        if "." in import_path:
            ...
            if len(package_parts) >= 2:
                ...
                return importlib.import_module(
                    f".{relative_import}", package="goobits_cli"
                )
        else:
    ...
    try:
        ...
        search_paths = [
            ...
            cli_dir.parent.parent.parent
            / configured_path,  # Three levels up (deep structure)
        ]
...
def get_version():
    ...
    try:
        ...
        possible_toml_paths = [
            ...
            / "pyproject.toml",  # For src/ structure
        ]
...
def main(ctx, verbose, interactive):
    """Build professional command-line tools with YAML configuration
        ...
        \b    [italic #B3B8C0]Transform simple YAML configuration into rich terminal applications with setup scripts, dependency management, and cross-platform compatibility.[/italic #B3B8C0]

        [italic #B3B8C0]Build professional command-line tools with YAML configuration[/italic #B3B8C0]
...
@click.option(
    "--universal-templates",
    help="🚀 Use Universal Template System (production-ready)",
    is_flag=True,
)
@click.pass_context
def build(ctx, config_path, output_dir, output, backup, universal_templates):
    ...
    try:
        if hooks is None:
            ...
            error_msg = "Hook implementation not found. Expected: app_hooks.py"
        ...
        if not hasattr(hooks, hook_name):
            available_hooks = [
                ...
                if not name.startswith("_") and callable(getattr(hooks, name))
            ]
            raise ConfigError(
        ...
        if "config_path" in expected_params or accepts_kwargs:
            # Only pass optional argument if it's not None, allowing function defaults to be used
        ...
        try:
            ...
            raise HookError(
                f"Hook function signature mismatch: {te}", hook_name=hook_name
            )
...
@click.option("--template", "-t", help="🎯 Template type", default="basic")
@click.option("--force", help="🔥 Overwrite existing goobits.yaml file", is_flag=True)
@click.pass_context
...
@click.option("--host", help="🌍 Host to bind the server to", default="localhost")
@click.option("--port", "-p", help="🔌 Port to run the server on", default=8080)
@click.pass_context

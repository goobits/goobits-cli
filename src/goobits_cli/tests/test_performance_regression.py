...
class TestPerformanceRegression:
    ...
    def test_basic_cli_startup_performance(self):
        ...
        assert import_time < 100, f"Basic CLI imports took {import_time:.1f}ms (target: <100ms)"
    ...
    def test_lazy_loading_overhead(self):
        ...
        assert callback_time < 50, f"Lazy loading overhead: {callback_time:.1f}ms (target: <50ms)"
...
'''
        ...
        try:
            ...
            result = subprocess.run([sys.executable, cli_path, '--interactive'], 
    ...
    def test_no_eager_universal_imports(self):
        ...
        try:
            ...
            assert import_time < 100, f"Universal template engine import: {import_time:.1f}ms (target: <100ms)"
    ...
    def test_generated_cli_startup_performance(self):
        ...
        start = time.perf_counter()
        result = subprocess.run([sys.executable, str(generated_cli_path), '--help'], 
        ...
        assert startup_time < 200, f"Generated CLI startup: {startup_time:.1f}ms (target: <200ms)"
...
def test_performance_targets():
    ...
    assert total_time < 100, f"Total CLI startup time: {total_time:.1f}ms (target: <100ms)"
    assert overhead_time < 50, f"Advanced features overhead: {overhead_time:.1f}ms (target: <50ms)"

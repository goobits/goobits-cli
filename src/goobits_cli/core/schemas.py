...
class InteractiveModeSchema(BaseModel):
    ...
    Provides a simple REPL (Read-Eval-Print Loop) for CLI applications
    with command history and tab completion support.
    """

    enabled: bool = True  # Master toggle for interactive mode
    history_enabled: bool = True  # Enable arrow-key command recall
    tab_completion: bool = True  # Enable tab completion for commands
    prompt: Optional[str] = None  # Custom prompt (defaults to "cli_name> ")
    max_history: int = 1000  # Maximum commands to remember

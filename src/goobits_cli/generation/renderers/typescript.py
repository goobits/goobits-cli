...
class TypeScriptGenerator(NodeJSGenerator):
    ...
    def __init__(self):
        ...
        template_dir = Path(__file__).parent.parent.parent / "templates" / "typescript"

        fallback_dir = Path(__file__).parent.parent.parent / "templates"
        ...
        # Add TypeScript-specific filters using shared utilities
        ...
        # Register filters - use shared utilities where available
        self.env.filters["json_stringify"] = shared_json_stringify
        self.env.filters["escape_backticks"] = escape_backticks
        self.env.filters["js_string"] = escape_javascript_string
...
3. Try with --debug flag for detailed logs"""
    ...
    def generate_all_files(
        ...
        context = {
            ...
            "description": getattr(
                config, "description", cli_config.description if cli_config else ""
            ),
            ...
            "bugs_url": (
                metadata.get("repository", "").replace(".git", "/issues")
        ...
        from ...universal.interactive.typescript_utils import (
            TypeScriptInteractiveRenderer,
        )
        ...
        if integrate_completion_system and get_completion_files_for_language:
            completion_files = get_completion_files_for_language(
                "typescript", context["command_name"]
            )

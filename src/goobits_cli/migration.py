...
class YAMLMigrationTool:
    ...
    def migrate_file(self, file_path: Path, backup: bool = True, dry_run: bool = False) -> bool:
        """
        ...
        try:
            ...
            if dry_run:
                console.print(f"🔍 [cyan]Dry run[/cyan] - No changes applied to {file_path}")
                return True
            ...
            with open(file_path, 'w', encoding='utf-8') as f:
                yaml.dump(migrated_data, f, default_flow_style=False, indent=2, sort_keys=False)
    ...
    def _migrate_structure(self, data: Dict[str, Any], file_path: Path) -> Dict[str, Any]:
        ...
        if isinstance(value, dict):
            ...
            if key == "subcommands" and isinstance(value, dict):
                ...
                for sub_key, sub_value in value.items():
                    migrated_subcommands[sub_key] = self._migrate_value(sub_value, sub_key, file_path)
                return migrated_subcommands
            else:
                # Regular dict, recurse
                migrated = {}
                for sub_key, sub_value in value.items():
                    migrated[sub_key] = self._migrate_value(sub_value, sub_key, file_path)
                return migrated
            ...
            if key == "subcommands" and self._is_subcommands_array(value):
                ...
                return [self._migrate_value(item, f"{key}[{i}]", file_path) for i, item in enumerate(value)]
    ...
    def _convert_subcommands_array_to_object(self, subcommands_array: List[Dict[str, Any]], file_path: Path) -> Dict[str, Any]:
    ...
    def _show_migration_summary(self, file_path: Path, original: Dict[str, Any], migrated: Dict[str, Any]):
    ...
    def _find_changes(self, original: Any, migrated: Any, path: str = "") -> List[Dict[str, str]]:
        ...
        if isinstance(original, dict) and isinstance(migrated, dict):
            ...
            for key in orig_keys & migr_keys:
                ...
                changes.extend(self._find_changes(original[key], migrated[key], key_path))
    ...
    def migrate_directory(self, directory: Path, pattern: str = "*.yaml", 
    ...
    def show_summary(self):
        ...
        if self.changes_made:
            ...
            console.print(Panel(panel_content, title="✅ Changes Made", border_style="green"))
        
        if self.warnings:
            panel_content = "\n".join(f"⚠️  {warning}" for warning in self.warnings)
            console.print(Panel(panel_content, title="⚠️  Warnings", border_style="yellow"))

    ...
    def _generate_legacy(self, config: Union[ConfigSchema, GoobitsConfigSchema], 
        ...
        for template_name in helper_templates:
            try:
                ...
                print(f"⚠️  Warning: Could not generate {template_name}")
                print(f"   Reason: {str(e)}")
                print(f"   Impact: The generated CLI will work but without {template_name.replace('.j2', '')} functionality")
                print(f"   Solution: Check template syntax or report this issue")

...
Generated from: complex_test.yaml
...
def _path_to_import_path(file_path: str, package_name: str = "complexcli") -> tuple[str, str]:
...
# No hooks path configured, try default locations
    ...
    try:
        return importlib.import_module(f"complexcli.{default_module_name}")
    except ImportError:
        pass

    # Strategy 2: Try relative import
    try:
        return importlib.import_module(f".{default_module_name}", package="complexcli")
    except ImportError:
        pass
...
def get_version():
    ...
    try:
        ...
        try:
            ...
            return pkg_resources.get_distribution("complexcli").version
        except:
            pass
...
class VersionedRichGroup(RichGroup):
    def format_usage(self, ctx, formatter):
        ...
        formatter.write_usage(f"complexcli v{get_version()}", " ".join(pieces))
...
@click.version_option(version=get_version(), prog_name=f"complexcli v{get_version()}")
@click.option(
    "--verbose", "-v",
...
@click.pass_context
def main(ctx, verbose, config_file, interactive):
    """Testing edge cases in Universal Template System

    \b
    [italic #B3B8C0]Complex test CLI for edge case testing[/italic #B3B8C0]
...
def upgrade(check, version, pre, dry_run):
    """Upgrade Complex CLI Test to the latest version"""
    import subprocess
    import shutil
    from pathlib import Path

    package_name = "complexcli"
    command_name = "complexcli"
    display_name = "Complex CLI Test"
...
@click.option(
    ...
    default=10)
@click.pass_context
def process(ctx, input_file, output_file, format, verbose, count):
    """Process files with multiple arguments and options"""    # Enhanced error handling for Python CLI
    ...
    try:
        if hooks is None:
            ...
            raise ConfigError(
                error_msg,
                suggestion=f"Create app_hooks.py with function: def on_process(...): pass"
            )
...
'count': count}
        ...
        if 'input_file' in expected_params or accepts_kwargs:
            call_kwargs['input_file'] = input_file
        if 'output_file' in expected_params or accepts_kwargs:
        ...
        if isinstance(result, int) and result != 0:
            click.echo(f"Command 'process' failed with exit code {result}", err=True)
            sys.exit(result)
...
@click.option(
    ...
    multiple=True)
@click.pass_context
def analyze(ctx, data_path, recursive, exclude):
    """Analyze data with complex options"""    # Enhanced error handling for Python CLI
    verbose_path = ['verbose']
    current_ctx = ctx
    verbose = False
    ...
    try:
        if hooks is None:
            ...
            raise ConfigError(
                error_msg,
                suggestion=f"Create app_hooks.py with function: def on_analyze(...): pass"
            )

        hook_name = 'on_analyze'
        if not hasattr(hooks, hook_name):
            available_hooks = [name for name in dir(hooks) if not name.startswith('_') and callable(getattr(hooks, name))]
            raise ConfigError(
                f"Hook function '{hook_name}' not found in app_hooks.py",
...
'exclude': exclude}
        ...
        # Add positional arguments as kwargs if function expects them
        if 'data_path' in expected_params or accepts_kwargs:
        ...
        # Handle return codes
        if isinstance(result, int) and result != 0:
            click.echo(f"Command 'analyze' failed with exit code {result}", err=True)
            sys.exit(result)

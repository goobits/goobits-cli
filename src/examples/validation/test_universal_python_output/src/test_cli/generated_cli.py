...
║  Generated on: 2025-08-25 19:08:14            ║
║                                                                              ║
...
def _path_to_import_path(
    file_path: str, package_name: str = "test_universal_cli"
) -> tuple[str, str]:
...
def _find_and_import_hooks():
    ...
    try:
        return importlib.import_module(f"test_universal_cli.{default_module_name}")
    except ImportError:
        pass
    ...
    try:
        return importlib.import_module(
            f".{default_module_name}", package="test_universal_cli"
        )
    except ImportError:
        pass
        ...
        for hooks_file in search_paths:
            if hooks_file.exists():
                spec = importlib.util.spec_from_file_location(
                    default_module_name, hooks_file
                )
...
def get_version():
    ...
    try:
        ...
        try:
            ...
            return version("test-universal-cli")
        ...
        try:
            ...
            return pkg_resources.get_distribution("test-universal-cli").version
        except Exception:
            pass
...
class VersionedRichGroup(RichGroup):
    def format_usage(self, ctx, formatter):
        ...
        formatter.write_usage(f"test-cli v{get_version()}", " ".join(pieces))
...
@click.version_option(version=get_version(), prog_name=f"test-cli v{get_version()}")
@click.option(
    "--verbose",
    "-v",
...
def main(ctx, verbose):
    """Test CLI for universal template validation

    \b    [italic #B3B8C0]A minimal test CLI to validate universal template generation[/italic #B3B8C0]

    [italic #B3B8C0]A test CLI for validating Universal Template System functionality[/italic #B3B8C0]
    """
    pass
...
def upgrade(check, version, pre, dry_run):
    ...
    display_name = "Test Universal CLI"
...
@main.command()
@click.argument("NAME")
@click.option("--greeting", "-g", help="Custom greeting message", default="Hello")
@click.pass_context
def hello(ctx, name, greeting):
    ...
    try:
        if hooks is None:
            ...
            raise ConfigError(
                error_msg,
                suggestion="Create app_hooks.py with function: def on_hello(...): pass",
            )

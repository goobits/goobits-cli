...
readonly PACKAGE_NAME="complex-cli-test"
readonly COMMAND_NAME="complex-cli"
readonly DISPLAY_NAME="Complex CLI Test Tool"
readonly DESCRIPTION="A complex CLI tool for testing code generation quality"
readonly PYPI_NAME="complex-cli-test"
...
readonly SHELL_ALIAS="complex-cli"
...
install_with_pipx() {
    ...
    if [[ "$install_dev" == "true" ]]; then
        ...
        (cd "$PROJECT_DIR" && pipx install --editable "$DEVELOPMENT_PATH[dev,test,lint]" --force) &
        ...
        pipx install "$PYPI_NAME[dev,test,lint]" --force &
...
install_with_pip() {
    ...
    if [[ "$install_dev" == "true" ]]; then
        ...
        (cd "$PROJECT_DIR" && python3 -m pip install --editable "$DEVELOPMENT_PATH[dev,test,lint]" --user) &
        ...
        python3 -m pip install "$PYPI_NAME[dev,test,lint]" --user &
...
upgrade_package() {
    ...
    if [[ "$PIPX_AVAILABLE" == "true" ]]; then
        ...
        python3 -m pip install --upgrade "$PYPI_NAME[dev,test,lint]" --user >/dev/null 2>&1 &
...
install_system_dependencies() {
    if command -v apt-get >/dev/null 2>&1; then
        ...
        tree_sub_node "info" "apt-get not found - manual installation required for: build-essential, git"
    fi
}



# NPM packages installation (after Python packages)
install_npm_extras() {
    if command -v npm >/dev/null 2>&1; then
        ...
        if [[ $? -eq 0 ]]; then
            tree_sub_node "success" "Installed npm package: prettier"
        else
            tree_sub_node "warning" "Failed to install npm package: prettier"
        fi
        
        npm install -g eslint >/dev/null 2>&1
        if [[ $? -eq 0 ]]; then
            tree_sub_node "success" "Installed npm package: eslint"
        else
            tree_sub_node "warning" "Failed to install npm package: eslint"
        fi

...
def builtin_upgrade_command(check_only=False, pre=False, version=None, dry_run=False):
    """Built-in upgrade function for Complex CLI Test Tool - uses enhanced setup.sh script."""
    import subprocess
    import sys
    from pathlib import Path

    if check_only:
        print(f"Checking for updates to Complex CLI Test Tool...")
        print("Update check not yet implemented. Run without --check to upgrade.")
        return

    if dry_run:
        print("Dry run - would execute: pipx upgrade complex-cli-test")
        return
    ...
    search_paths = [
        ...
        Path.home() / ".local" / "share" / "complex-cli-test" / "setup.sh",  # User data
    ...
    if setup_script is None:
        ...
        print(f"Enhanced setup script not found. Using basic upgrade for Complex CLI Test Tool...")
        import shutil
        
        package_name = "complex-cli-test"
        pypi_name = "complex-cli-test"
        ...
        if result.returncode == 0:
            print(f"✅ Complex CLI Test Tool upgraded successfully!")
            print(f"Run 'complex-cli --version' to verify the new version.")
        else:
...
def load_plugins(cli_group):
    ...
    plugin_dirs = [
        ...
        Path.home() / ".config" / "complex-cli-test" / "plugins",
...
@click.version_option(version=get_version(), prog_name="complex-cli")
@click.pass_context
...
def main(ctx):
    """[bold color(6)]complex-cli v1.0.0[/bold color(6)] - Test complex code generation patterns

    
    \b
    [#B3B8C0]Advanced CLI with multiple commands, subcommands, and features[/#B3B8C0]
...
def upgrade(check, version, pre, dry_run):
    """Upgrade Complex CLI Test Tool to the latest version."""
...
def build(ctx, target, source, verbose, output_format, parallel_jobs, timeout):
...
@click.option("-f", "--force",
    is_flag=True,
    help="Force deployment ignoring checks"
)

def deploy(ctx, environment, dry_run, config_file, force):
    """Deploy application to various environments"""
    
    # Check for built-in commands first
    
    # Standard command - use the existing hook pattern
    hook_name = f"on_deploy"
    if app_hooks and hasattr(app_hooks, hook_name):
        ...
        kwargs = {}
        kwargs['command_name'] = 'deploy'  # Pass command name for all commands
        ...
        click.echo(f"Executing deploy command...")
...
def unit(ctx, coverage, pattern):
    """Run unit tests"""
    # Check if hook function exists
    hook_name = f"on_test_unit"
    if app_hooks and hasattr(app_hooks, hook_name):
        # Call the hook with all parameters
        hook_func = getattr(app_hooks, hook_name)
        ...
        # Prepare arguments including global options
        kwargs = {}
        kwargs['command_name'] = 'unit'  # Pass command name for all commands
        ...
        # Default placeholder behavior
        click.echo(f"Executing unit command...")
...
@click.option("--environment",
    type=click.Choice(['test', 'staging']),
    default="test",
...
def integration(ctx, service, environment, parallel):
    """Run integration tests"""
    # Check if hook function exists
    hook_name = f"on_test_integration"
    if app_hooks and hasattr(app_hooks, hook_name):
        # Call the hook with all parameters
        hook_func = getattr(app_hooks, hook_name)
        ...
        # Prepare arguments including global options
        kwargs = {}
        kwargs['command_name'] = 'integration'  # Pass command name for all commands
        ...
        # Default placeholder behavior
        click.echo(f"Executing integration command...")
...
@click.option("--threshold",
    type=float,
    default=1000.0,
    help="Performance threshold in ms"
)

def performance(ctx, iterations, threshold):
    """Run performance benchmarks"""
    # Check if hook function exists
    hook_name = f"on_test_performance"
    if app_hooks and hasattr(app_hooks, hook_name):
        # Call the hook with all parameters
        hook_func = getattr(app_hooks, hook_name)
        ...
        # Prepare arguments including global options
        kwargs = {}
        kwargs['command_name'] = 'performance'  # Pass command name for all commands
        ...
        # Default placeholder behavior
        click.echo(f"Executing performance command...")
...
@click.option("-g", "--global",
    is_flag=True,
    help="Set global configuration"
)

def set(ctx, key, value, global):
...
def instructions(shell):
    ...
    click.echo(f"   complex-cli completion generate {shell} > completion.{shell}")

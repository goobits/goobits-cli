#!/usr/bin/env python3
"""Auto-generated from test_interactive.yaml"""
import os
import sys
import logging
...
def builtin_upgrade_command(check_only=False, pre=False, version=None, dry_run=False):
    """Built-in upgrade function for Test Interactive CLI - uses enhanced setup.sh script."""
    import subprocess
    import sys
    from pathlib import Path

    if check_only:
        print(f"Checking for updates to Test Interactive CLI...")
        print("Update check not yet implemented. Run without --check to upgrade.")
        return

    if dry_run:
        print("Dry run - would execute: pipx upgrade test-interactive-cli")
        return
    ...
    search_paths = [
        ...
        Path.home() / ".local" / "share" / "test-interactive-cli" / "setup.sh",  # User data
    ...
    if setup_script is None:
        ...
        print(f"Enhanced setup script not found. Using basic upgrade for Test Interactive CLI...")
        import shutil
        
        package_name = "test-interactive-cli"
        pypi_name = "test-interactive-cli"
        ...
        if result.returncode == 0:
            print(f"✅ Test Interactive CLI upgraded successfully!")
            print(f"Run 'test-cli --version' to verify the new version.")
        else:
...
def load_plugins(cli_group):
    ...
    plugin_dirs = [
        ...
        Path.home() / ".config" / "test-interactive-cli" / "plugins",
...
@click.version_option(version=get_version(), prog_name="test-cli")
@click.pass_context
...
def main(ctx, interactive=False):
    ...
    \b
    [#B3B8C0]A simple CLI to test interactive mode functionality[/#B3B8C0]
...
def upgrade(check, version, pre, dry_run):
    """Upgrade Test Interactive CLI to the latest version."""
...
def hello(ctx, name, loud):
    ...
    if app_hooks and hasattr(app_hooks, hook_name):
        ...
        kwargs = {}
        kwargs['command_name'] = 'hello'  # Pass command name for all commands
        ...
        click.echo(f"Executing hello command...")
...
def goodbye(ctx, name):
    ...
    if app_hooks and hasattr(app_hooks, hook_name):
        ...
        # Prepare arguments including global options
        kwargs = {}
        kwargs['command_name'] = 'goodbye'  # Pass command name for all commands
        ...
        # Default placeholder behavior
        click.echo(f"Executing goodbye command...")
...
def instructions(shell):
    ...
    click.echo(f"   test-cli completion generate {shell} > completion.{shell}")

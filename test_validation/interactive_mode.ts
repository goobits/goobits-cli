...
class TesttypescriptcliInteractive {
    ...
    private loadCommandHistory(): void {
        const historyFile = path.join(process.cwd(), '.testcli_history');
        try {
    ...
    private async handleGreet(args: string[]): Promise<void> {
        const hookName = 'onGreet' as keyof HookRegistry;
        const hook = hooks[hookName];
        
        if (typeof hook === 'function') {
            try {
                // Parse arguments with type validation
                const parsedContext = await this.parseCommandContext('greet', args);
                ...
                if (hasTypeScriptCompiler && this.typeChecker) {
                    const suggestions = this.typeChecker.getSuggestions(error as Error, 'greet', args);
                    if (suggestions.length > 0) {
                        console.log('💡 Suggestions:');
                        suggestions.forEach(suggestion => console.log(`   • ${suggestion}`));
                    }

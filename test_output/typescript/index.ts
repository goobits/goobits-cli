 ...
 * Main CLI implementation for TestTypeScriptCLI
 * 
 * Features:
...
const runtimeConfig: Partial<ConfigSchema> = {
  ...
  command_name: 'testts',
  display_name: 'TestTypeScriptCLI',
  description: 'Test TypeScript CLI for verification',
  version: '1.0.0',
};
...
async function loadPlugins(program: Command): Promise<Result<number, PluginError[]>> {
  const pluginDirs: string[] = [
    ...
    join(process.env.HOME || '', '.config', 'goobits', 'TestTypeScriptCLI', 'plugins'),
...
async function builtinUpgradeCommand(options: UpgradeOptions): Promise<void> {
  ...
  const displayName: string = 'TestTypeScriptCLI';
  ...
  try {
    ...
    console.log(chalk.gray(`Run 'testts --version' to verify the new version.`));
  } catch (error: any) {

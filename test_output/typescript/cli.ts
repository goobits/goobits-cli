#!/usr/bin/env node
/**
 * Auto-generated from test_typescript_valid.yaml
 * TestTypeScriptCLI CLI - TypeScript Implementation
 */
...
async function builtinUpgradeCommand(checkOnly: boolean = false, pre: boolean = false, version: string | null = null, dryRun: boolean = false): Promise<void> {
    if (checkOnly) {
        console.log(`Checking for updates to TestTypeScriptCLI...`);
        console.log('Update check not yet implemented. Run without --check to upgrade.');
        return;
    }

    if (dryRun) {
        console.log('Dry run - would execute: npm update -g test-typescript-cli');
        return;
    }
    ...
    const searchPaths = [
        ...
        path.join(__dirname, '../setup.sh'),
        path.join(process.env.HOME || process.env.USERPROFILE || '', '.local', 'share', 'test-typescript-cli', 'setup.sh')
    ];
    ...
    if (!setupScript) {
        ...
        console.log(`Enhanced setup script not found. Using basic upgrade for TestTypeScriptCLI...`);
        
        const packageName = 'test-typescript-cli';
        const npmName = 'test-typescript-cli';
        
        try {
            const cmd = process.platform === 'win32' ? 'npm.cmd' : 'npm';
            execSync(`${cmd} update -g ${npmName}`, { stdio: 'inherit' });
            console.log(`✅ TestTypeScriptCLI upgraded successfully!`);
            console.log(`Run 'testts --version' to verify the new version.`);
        } catch (e) {
...
function loadPlugins(program: Command): void {
    const pluginDirs = [
        path.join(process.env.HOME || process.env.USERPROFILE || '', '.config', 'goobits', 'TestTypeScriptCLI', 'plugins'),
...
// Configure program
program
    .name('TestTypeScriptCLI')
    .version(getVersion())
    .description(`TestTypeScriptCLI v${getVersion()} - Test TypeScript CLI for verification`)
...
program
    .command('upgrade')
    .description('Upgrade TestTypeScriptCLI to the latest version')
...
completionCmd
    ...
    .action((shell: string) => {
        ...
        console.log(`   TestTypeScriptCLI completion generate ${shell} > completion.${shell}`);

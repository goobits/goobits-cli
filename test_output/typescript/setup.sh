#!/bin/bash
# Enhanced TypeScript setup script for TestTypeScriptCLI
...
setup_config_dir() {
    ...
    case "$OSTYPE" in
        darwin*)
            config_dir="$HOME/Library/Application Support/test-typescript-cli"
            ;;
        msys*|win32*)
            config_dir="${APPDATA:-$HOME}/test-typescript-cli"
            ;;
        *)
            config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/test-typescript-cli"
            ;;
    esac
...
uninstall() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/test-typescript-cli"
        if [ -d "$config_dir" ]; then
...
show_help() {
    echo ""
    echo -e "${BLUE}TestTypeScriptCLI TypeScript Setup Script${NC}"
    echo "====================================="
    echo ""

...
impl DynamicPlugin {
    ...
    pub fn load(library_path: &Path, metadata: PluginMetadata) -> Result<Self> {
        unsafe {
            let library = Library::new(library_path)?;
            let create_plugin: Symbol<fn() -> Box<dyn Plugin>> = library
                .get(metadata.entry_point.as_bytes())
                .with_context(|| format!("Failed to find entry point '{}' in plugin", metadata.entry_point))?;
...
impl Plugin for CorePlugin {
    ...
    fn description(&self) -> &str {
        "Core functionality for TestRustCLI"
    }
...
impl Command for ConfigGetCommand {
    ...
    fn execute(&self, args: &CommandArgs) -> Result<()> {
        if let Some(key) = args.get_arg(0) {
            ...
            anyhow::bail!("Configuration key is required");
        }
        Ok(())
    }
...
impl Command for ConfigSetCommand {
    ...
    fn validate(&self, args: &CommandArgs) -> Result<()> {
        ...
        if args.get_arg(1).is_none() {
            anyhow::bail!("Configuration value is required");
        }
        Ok(())
    }
}
...
impl PluginRegistry {
    ...
    fn load_dynamic_plugins(&mut self, config: &AppConfig) -> Result<()> {
    ...
    fn load_plugin_from_manifest(&mut self, manifest_path: &Path, config: &AppConfig) -> Result<()> {
    ...
    pub fn execute_command(&self, name: &str, args: &CommandArgs) -> Result<()> {
        if let Some(command) = self.get_command(name) {
            command.validate(args)?;
            command.execute(args)
        } else {
            anyhow::bail!("Unknown plugin command: {}", name)
        }
    }
...
pub fn initialize_plugins(config: &AppConfig) -> Result<()> {
...
pub fn execute_plugin_command(command: &str, args: &CommandArgs) -> Result<()> {
    ...
    if let Some(ref registry) = *global_registry {
        ...
        anyhow::bail!("Plugin registry not initialized")
    }
}

 ...
 * Main CLI implementation for TestRustCLI
 */
...
#[derive(Parser)]
#[command(name = "testrust")]
#[command(about = "Test Rust CLI for verification")]
#[command(long_about = "Test Rust CLI for verification")]
...
enum Commands {
    ...
    /// Upgrade TestRustCLI to the latest version
    Upgrade {
...
fn handle_upgrade(check: bool, version: Option<String>, pre: bool, dry_run: bool) -> CliResult<()> {
    ...
    if status.success() {
        ...
        utils::output::info(&format!("Run '{} --version' to verify the new version.", "testrust"));
    } else {
...
fn main() {
    ...
    std::panic::set_hook(Box::new(|panic_info| {
        eprintln!("💥 TestRustCLI encountered an unexpected error:");
        eprintln!("{}", panic_info);
    ...
    match shell {
        "bash" => {
            if system {
                ...
                println!("   {} completion generate bash | sudo tee /etc/bash_completion.d/{}", "testrust", "testrust");
                println!();
                ...
                println!("   {} completion generate bash > ~/.local/share/bash-completion/completions/{}", "testrust", "testrust");
                println!();
                println!("3. Add to ~/.bashrc:");
                println!("   source ~/.local/share/bash-completion/completions/{}",  "testrust");
            }
        }
        "zsh" => {
            if system {
                println!("1. Generate completion script:");
                println!("   {} completion generate zsh | sudo tee /usr/local/share/zsh/site-functions/_{}", "testrust", "testrust");
                println!();
                ...
                println!();
                println!("2. Generate completion script:");
                println!("   {} completion generate zsh > ~/.local/share/zsh/site-functions/_{}", "testrust", "testrust");
                println!();
        ...
        "fish" => {
            if system {
                println!("1. Generate completion script:");
                println!("   {} completion generate fish | sudo tee /usr/share/fish/completions/{}.fish", "testrust", "testrust");
            } else {
                println!("1. Create completion directory:");
                println!("   mkdir -p ~/.config/fish/completions");
                println!();
                println!("2. Generate completion script:");
                println!("   {} completion generate fish > ~/.config/fish/completions/{}.fish", "testrust", "testrust");
            }
            println!();
        ...
        "powershell" => {
            println!("1. Generate completion script:");
            println!("   {} completion generate powershell > {}_completion.ps1", "testrust", "testrust");
            println!();
            println!("2. Add to PowerShell profile:");
            println!("   . ./{}_completion.ps1", "testrust");
            println!();
...
"#, "testrust", "testrust", "testrust", "testrust", "testrust")
}

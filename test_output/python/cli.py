#!/usr/bin/env python3
"""Auto-generated from test_python_valid.yaml"""
import os
import sys
import logging
...
def builtin_upgrade_command(check_only=False, pre=False, version=None, dry_run=False):
    """Built-in upgrade function for TestPythonCLI - uses enhanced setup.sh script."""
    import subprocess
    import sys
    from pathlib import Path

    if check_only:
        print(f"Checking for updates to TestPythonCLI...")
        print("Update check not yet implemented. Run without --check to upgrade.")
        return

    if dry_run:
        print("Dry run - would execute: pipx upgrade test-python-cli")
        return
    ...
    search_paths = [
        ...
        Path.home() / ".local" / "share" / "test-python-cli" / "setup.sh",  # User data
    ...
    if setup_script is None:
        ...
        print(f"Enhanced setup script not found. Using basic upgrade for TestPythonCLI...")
        import shutil
        ...
        if result.returncode == 0:
            print(f"✅ TestPythonCLI upgraded successfully!")
            print(f"Run 'testpy --version' to verify the new version.")
        else:
...
def load_plugins(cli_group):
    ...
    plugin_dirs = [
        ...
        Path.home() / ".config" / "goobits" / "TestPythonCLI" / "plugins",
...
@click.version_option(version=get_version(), prog_name="TestPythonCLI")
@click.pass_context



def main(ctx):
    """[bold color(6)]TestPythonCLI v1.0.0[/bold color(6)] - Test Python CLI for verification
...
def upgrade(check, version, pre, dry_run):
    """Upgrade TestPythonCLI to the latest version."""
...
def hello(ctx, name, greeting):
    ...
    if app_hooks and hasattr(app_hooks, hook_name):
        ...
        click.echo(f"  greeting: {greeting}")
...
def instructions(shell):
    ...
    click.echo(f"   testpythoncli completion generate {shell} > completion.{shell}")

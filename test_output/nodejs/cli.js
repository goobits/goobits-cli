 ...
 * TestNodeJSCLI CLI - Node.js Implementation
 */
...
try {
    ...
    if (process.env.DEBUG) console.debug('Config manager not available:', e.message);
}
...
try {
    ...
    if (process.env.DEBUG) console.debug('Progress helper not available:', e.message);
}
...
try {
    ...
    if (process.env.DEBUG) console.debug('Prompts helper not available:', e.message);
}
...
try {
    ...
    if (process.env.DEBUG) console.debug('Completion helper not available:', e.message);
}
...
async function builtinUpgradeCommand(checkOnly = false, pre = false, version = null, dryRun = false) {
    if (checkOnly) {
        console.log(`Checking for updates to TestNodeJSCLI...`);
        console.log('Update check not yet implemented. Run without --check to upgrade.');
        return;
    }

    if (dryRun) {
        console.log('Dry run - would execute: npm update -g test-nodejs-cli');
        return;
    }
    ...
    const searchPaths = [
        ...
        path.join(__dirname, '../setup.sh'),
        path.join(process.env.HOME, '.local', 'share', 'test-nodejs-cli', 'setup.sh')
    ];
    ...
    if (!setupScript) {
        ...
        console.log(`Enhanced setup script not found. Using basic upgrade for TestNodeJSCLI...`);
        
        const packageName = 'test-nodejs-cli';
        const npmName = 'test-nodejs-cli';
        
        try {
            const cmd = process.platform === 'win32' ? 'npm.cmd' : 'npm';
            execSync(`${cmd} update -g ${npmName}`, { stdio: 'inherit' });
            console.log(`✅ TestNodeJSCLI upgraded successfully!`);
            console.log(`Run 'testnjs --version' to verify the new version.`);
        } catch (e) {
...
function loadPlugins(program) {
    const pluginDirs = [
        path.join(process.env.HOME || process.env.USERPROFILE, '.config', 'goobits', 'TestNodeJSCLI', 'plugins'),
...
program
    .name('TestNodeJSCLI')
    .version(getVersion())
    .description(`TestNodeJSCLI v${getVersion()} - Test Node.js CLI for verification`)
...
program
    .command('upgrade')
    .description('Upgrade TestNodeJSCLI to the latest version')
...
const helloCmd = program
    ...
    .option('-g, --greeting <value>', 'Custom greeting', 'Hello')
    ...
    .action(async (name, options, command) => {
        try {
            ...
            if (appHooks && typeof appHooks[hookName] === 'function') {
                ...
                await appHooks[hookName](kwargs);
            } else {
                ...
                console.log(`  greeting: ${options.greeting}`);
...
completionCmd
    ...
    .action((shell) => {
        ...
        console.log(`   TestNodeJSCLI completion generate ${shell} > completion.${shell}`);
...
program
    ...
    .action((shell, currentLine, cursorPos, options) => {
        try {
...
module.exports = { program, cliEntry };

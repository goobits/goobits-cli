#!/bin/bash
# Enhanced setup script for TestNodeJSCLI
...
COMMAND_NAME="testnjs"
DISPLAY_NAME="TestNodeJSCLI"
VERSION="1.0.0"
DESCRIPTION="Test Node.js CLI for verification"
NPM_NAME="test-nodejs-cli"
...
is_development() {
    [ -f "package.json" ] && [ -f "testnodejscli.js" -o -f "cli.js" ]
}
...
setup_config_dir() {
    ...
    case "$OSTYPE" in
        darwin*)
            config_dir="$HOME/Library/Application Support/test-nodejs-cli"
            ;;
        msys*|win32*)
            config_dir="${APPDATA:-$HOME}/test-nodejs-cli"
            ;;
        *)
            config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/test-nodejs-cli"
            ;;
    esac
...
uninstall() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/test-nodejs-cli"
        if [ -d "$config_dir" ]; then
...
show_help() {
    echo ""
    echo -e "${BLUE}TestNodeJSCLI Setup Script${NC}"
    echo "===================================="
    echo ""

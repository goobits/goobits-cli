 ...
 * Main CLI implementation for TestNodeJSCLI
 */
...
const loadPlugins = asyncErrorHandler(async function loadPlugins(program) {
  try {
    ...
    const pluginDirs = [
      ...
      join(process.env.HOME || '', '.config', 'goobits', 'TestNodeJSCLI', 'plugins'),
...
async function builtinUpgradeCommand(options) {
  ...
  const displayName = 'TestNodeJSCLI';
  ...
  try {
    ...
    console.log(chalk.gray(`Run 'testnjs --version' to verify the new version.`));
  } catch (error) {
...
function generateDynamicCompletion(shell) {
  ...
  switch (shell) {
    case 'bash':
      console.log(`# Basic bash completion for testnjs
_testnjs_completions() {
...
complete -F _testnjs_completions testnjs`);
      break;
      
    case 'zsh':
      console.log(`#compdef testnjs
# Basic zsh completion for testnjs
_testnjs() {
    local commands=(
...
_testnjs "$@"`);
    ...
    case 'fish':
      console.log(`# Basic fish completion for testnjs
complete -c testnjs -f
complete -c testnjs -l help -s h -d 'Show help information'
complete -c testnjs -l version -s V -d 'Show version information'

${commands.split(' ').map(cmd => 
  `complete -c testnjs -n '__fish_use_subcommand' -a ${cmd} -d '${cmd} command'`
).join('\n')}`);
      break;
  }
}
...
export const cli = asyncErrorHandler(async function cli() {
  ...
  program
    .name('TestNodeJSCLI')
    .description(`Test Node.js CLI for verification`)
    .version(version)

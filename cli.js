 ...
 * Test 3-Level Commands CLI - Node.js Implementation
 */
...
async function builtinUpgradeCommand(checkOnly = false, pre = false, version = null, dryRun = false) {
    if (checkOnly) {
        console.log(`Checking for updates to Test 3-Level Commands...`);
        console.log('Update check not yet implemented. Run without --check to upgrade.');
        return;
    }

    if (dryRun) {
        console.log('Dry run - would execute: npm update -g test-3level');
        return;
    }
    ...
    const searchPaths = [
        ...
        path.join(__dirname, '../setup.sh'),
        path.join(process.env.HOME, '.local', 'share', 'test-3level', 'setup.sh')
    ];
    ...
    if (!setupScript) {
        ...
        console.log(`Enhanced setup script not found. Using basic upgrade for Test 3-Level Commands...`);
        
        const packageName = 'test-3level';
        const npmName = 'test-3level';
        
        try {
            const cmd = process.platform === 'win32' ? 'npm.cmd' : 'npm';
            execSync(`${cmd} update -g ${npmName}`, { stdio: 'inherit' });
            console.log(`✅ Test 3-Level Commands upgraded successfully!`);
            console.log(`Run 'test3level --version' to verify the new version.`);
        } catch (e) {
...
async function loadPlugins(program) {
    const pluginDirs = [
        path.join(process.env.HOME || process.env.USERPROFILE, '.config', 'goobits', 'test3level', 'plugins'),
...
program
    .name('test3level')
    .version(getVersion())
    .description(`test3level v${getVersion()} - Test 3-level nesting`)
...
const simpleCmd = program
    ...
    .action(async (messageoptions, command) => {
        try {
    ...
    .action(async (options, command) => {
        try {
            ...
            if (hooks && typeof hooks[hookName] === 'function') {
                ...
                // Default placeholder behavior
                console.log(`Executing v1 command...`);
            }
        } catch (error) {
    ...
    .action(async (options, command) => {
        try {
            ...
            if (hooks && typeof hooks[hookName] === 'function') {
                ...
                // Default placeholder behavior
                console.log(`Executing level2 command...`);
            }
        } catch (error) {
            handleCLIError(error, options.verbose || command.parent.parent.opts().verbose);
            process.exit(error.exitCode || 1);
...
builtin_completionCmd
    ...
    .action((shell) => {
        ...
        console.log(`   test3level completion generate ${shell} > completion.${shell}`);

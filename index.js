 ...
 * Main CLI implementation for TestNodeCLI
 */
...
const loadPlugins = asyncErrorHandler(async function loadPlugins(program) {
  try {
    ...
    const pluginDirs = [
      ...
      join(process.env.HOME || '', '.config', 'goobits', 'TestNodeCLI', 'plugins'),
...
async function builtinUpgradeCommand(options) {
  ...
  try {
    ...
    console.log(chalk.gray(`Run 'testnodecli --version' to verify the new version.`));
  } catch (error) {
...
function generateDynamicCompletion(shell) {
  ...
  switch (shell) {
    case 'bash':
      console.log(`# Basic bash completion for testnodecli
...
complete -F _testnodecli_completions testnodecli`);
    ...
    case 'zsh':
      console.log(`#compdef testnodecli
# Basic zsh completion for testnodecli
_testnodecli() {
    local commands=(
...
_testnodecli "$@"`);
    ...
    case 'fish':
      console.log(`# Basic fish completion for testnodecli
complete -c testnodecli -f
...
${commands.split(' ').map(cmd => 
  `complete -c testnodecli -n '__fish_use_subcommand' -a ${cmd} -d '${cmd} command'`
).join('\n')}`);
      break;
  }
}
...
export const cli = asyncErrorHandler(async function cli() {
  ...
  program
    .name('TestNodeCLI')
    .description(`Node.js test CLI for validation`)
    .version(version)

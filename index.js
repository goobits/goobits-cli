 ...
 * Main CLI implementation for Test 3-Level Commands
 */
...
const loadPlugins = asyncErrorHandler(async function loadPlugins(program) {
  try {
    ...
    const pluginDirs = [
      ...
      join(process.env.HOME || '', '.config', 'goobits', 'test3level', 'plugins'),
...
async function builtinUpgradeCommand(options) {
  ...
  const packageName = 'test-3level';
  const displayName = 'Test 3-Level Commands';
  ...
  try {
    ...
    console.log(chalk.gray(`Run 'test3level --version' to verify the new version.`));
  } catch (error) {
...
function generateDynamicCompletion(shell) {
  ...
  switch (shell) {
    case 'bash':
      console.log(`# Basic bash completion for test3level
...
complete -F _test3level_completions test3level`);
    ...
    case 'zsh':
      console.log(`#compdef test3level
# Basic zsh completion for test3level
_test3level() {
    local commands=(
...
_test3level "$@"`);
    ...
    case 'fish':
      console.log(`# Basic fish completion for test3level
complete -c test3level -f
...
${commands.split(' ').map(cmd => 
  `complete -c test3level -n '__fish_use_subcommand' -a ${cmd} -d '${cmd} command'`
).join('\n')}`);
      break;
  }
}
...
export const cli = asyncErrorHandler(async function cli() {
  ...
  program
    .name('test3level')
    .description(`Test 3-level nesting`)
    .version(version)
  ...
  program
    .command('upgrade')
    .description('Upgrade Test 3-Level Commands to the latest version')
...
;
    ...
    cmd.action(async (message, options, command) => {
      ...
      if (appHooks && appHooks[hookName]) {
        ...
        try {
          ...
          const hookError = error instanceof CLIError ? error : new CLIError(
            ...
            `Failed to execute command 'simple'. ${error.message}`,
            1
          );
        ...
        if (appHooks && appHooks[hookName]) {
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              ...
              `Failed to execute subcommand 'v1'. ${error.message}`,
              1
            );
          ...
          // Default placeholder behavior
          console.log(chalk.blue(`Executing v1 subcommand...`));
  ...
  }
  {
    ...
    const parentCmd = program
      .command('nested')
      .description('Nested command test');

    {
      const subCmd = parentCmd
        .command('level2')
        .description('Level 2 subcommand');



      subCmd.action(async (options, command) => {
        const hookName = 'onNestedLevel2';
        if (appHooks && appHooks[hookName]) {
          const hookFunc = appHooks[hookName];
          ...
          try {
            ...
            const subcommandError = error instanceof CLIError ? error : new CLIError(
              `Subcommand hook '${hookName}' failed: ${error.message}`,
              'SUBCOMMAND_EXECUTION_ERROR',
              `Failed to execute subcommand 'level2'. ${error.message}`,
              1
            );
            handleError(subcommandError);
          }
          ...
          // Default placeholder behavior
          console.log(chalk.blue(`Executing level2 subcommand...`));

...
readonly PACKAGE_NAME="test_cli_python"
readonly COMMAND_NAME="test_cli_python"
readonly DISPLAY_NAME="Test CLI Python"
readonly DESCRIPTION="End-to-end integration test CLI for Python"
readonly PYPI_NAME="test-python"
...
readonly SHELL_ALIAS="test_cli_python"

# Dependencies (legacy format for backward compatibility)
readonly REQUIRED_DEPS=("pipx")
...
install_with_pipx() {
    ...
    if [[ "$install_dev" == "true" ]]; then
        ...
        (cd "$PROJECT_DIR" && pipx install --editable "$DEVELOPMENT_PATH[click,rich]" --force) &
        local install_pid=$!
        ...
        pipx install "$PYPI_NAME[click,rich]" --force &
        local install_pid=$!
...
install_with_pip() {
    ...
    if [[ "$install_dev" == "true" ]]; then
        ...
        (cd "$PROJECT_DIR" && python3 -m pip install --editable "$DEVELOPMENT_PATH[click,rich]" --user) &
        show_spinner $!
        wait $!
        local exit_code=$?
        ...
        python3 -m pip install "$PYPI_NAME[click,rich]" --user &
        show_spinner $!
        wait $!
        local exit_code=$?
...
upgrade_package() {
    ...
    if [[ "$PIPX_AVAILABLE" == "true" ]]; then
        ...
        python3 -m pip install --upgrade "$PYPI_NAME[click,rich]" --user >/dev/null 2>&1 &
        show_spinner $!
        wait $!
        local exit_code=$?
    fi
